...
- [x] **Code / Git / Run menu Alt-shortcuts moved (2026-07-20)** — the natural mnemonics `Alt+C`, `Alt+G`, `Alt+R` silently do nothing on the machines we have (defect reproduces in upstream VisualFBEditor in both 32- and 64-bit builds; root cause unresolved, tracked as ROADMAP §13.28 pt 3). As a **workaround, not a fix**, the three menus are captioned *Co**d**e* (Alt+D), *G**i**t* (Alt+I), *R**u**n* (Alt+U). Every other menu keeps its natural shortcut. Verified by effect: all three new letters open their menus alongside the six controls. The swallowed letters will be restored when the underlying defect is understood — the investigation is paused pending a proper two-machine lab, and `MffMnemonicTest.bas` + `MenuProbe.ps1` + framework instrumentation remain in place for it.

- [x] **Tip of the Day removed; toolbars fixed to three rows and made all-or-nothing (2026-07-19)** — owner decision, three UI simplifications made together because they share the same startup and chrome code. Each removes a choice rather than adding one. **Tip of the Day** is gone entirely: Help menu item, startup modal, command case, `#include`, both globals, `src/frmTipOfDay.frm` and `Help/Tip of the Day/` (12 files); its two INI keys are actively `KeyRemove`d so they do not linger in existing settings. **The toolbars are pinned to three rows** — Standard+Edit+Project / Run / Format — via `RBBS_BREAK`, regardless of window or monitor width; previously the ReBar reflowed on resize and a wide monitor collapsed everything onto one long line of icons. The subtlety worth remembering: *maximising a band makes it fill its row*, so the old loop maximising bands 0–3 would have pushed Standard's and Edit's row-mates onto rows of their own and defeated the breaks; it now maximises only the last band on each row. **View ▸ Toolbars is one toggle**, not a submenu of five, backed by a single `ShowToolBars` key with the five retired keys removed; right-clicking a toolbar no longer does anything, as that handler existed only to pop the submenu. Net 296 lines deleted / 117 added; clean build, zero warnings, D1 re-run 12/12, all three owner-verified including that the three rows hold when maximised. Detail in ROADMAP §13.31.

- [x] **`Documentation/` folder removed (2026-07-16, `2d833f4`)** — deleted 1092 loose HTML pages (~15 MB) that were a redundant copy of the FreeBASIC language reference. `Help/FB-manual-en_US-1.10.1.chm` already ships the same content in the IDE's compiled help form, and the folder was already excluded from releases, so it was pure duplication. Nothing in code/build/help pointed into it; StageRelease.ps1's explanatory comment updated to past tense. Staging re-verified clean afterward (3,771 files, 302 MB, `Documentation/` absent, Help CHM intact).

- [x] **Dark mode removed entirely (2026-07-15)** — owner decision, for stability, after a dark-mode-only context-menu render bug (dark popup submenus after the first painted empty; instrumentation proved the menu data was correct, so it was purely an owner-draw paint fault) on top of the T10/T11/T12 history of dark-only glitches. Removed in safety-first waves (pin `g_darkModeSupported = False` → verify light-only build → delete dead code, compiling between each): the Tools ▸ Options Dark Mode checkbox + INI setting, the framework menu owner-draw system (Control/Form/ToolBar) that caused the bug, the whole `SetDark` virtual hierarchy (11 methods / 13 declarations / all triggers), every per-control dark custom-draw branch, the `DarkMode/` module (deleted), and the dark colour/brush globals. Net: 68 files changed, ~2,650 lines removed, 4 files deleted, 0 dark-mode references remaining; build clean, owner-verified. User ForeColor/BackColor custom-draw preserved (CheckBox/RadioButton condition simplified to the `FForeColor` path, disabled-text grey inlined as `&H626262`); `nullptr` (defined in the deleted header) replaced with `NULL`. Full detail in `PROJECT_STATUS.md`. Supersedes T10/T11/T12 (retained there as historical record).

- [x] **Temporary example/project artifacts cleaned up and shared settings synchronized** — deleted tracked scratch files (`Controls/Framework/mff/Temp.bas`, `Examples/DeviceExplorer/Temp.bas`), removed the obsolete `Examples/DeviceExplorer/DeviceExplorer.vfp`, cleared repository-root `Temp/` backups, and committed the current `Settings/astoria.ini`. The tracked INI is intentionally kept synchronized through commits because development alternates between two computers.
...
- [x] **MFF hygiene pass closed** — deleted the dead Chinese-language leftovers `README_CN.md` and `changes_cn.txt`, with their two references (`MyFbFramework.vfp`'s file list, `README.md`'s language-switcher link) cleaned up. The other two items on this list — MFF control-library path consolidation and the standalone-Canvas device-ownership fix — were dropped, not attempted: both are gated on conditions that don't hold today (a standing "Do Not Attempt" review verdict, and a test harness that doesn't exist), documented in `PROJECT_STATUS.md`.
- [x] **MFF DLL renamed to `astoria.dll`, moved to repo root** — owner request. Source file names untouched; only the compiled artifact moved, from `Controls/MyFbFramework/mff64.dll` to `astoria.dll` next to `astoria.exe`. Required updating `Controls/MyFbFramework/Settings.ini`'s header/source/include/lib-folder keys to explicit root-relative paths, since the Designer's Toolbox/component-discovery system resolves all of those relative to wherever the DLL physically sits — a naive rename would have reproduced the grey-panel Form Designer bug class. Also fixed a stale pre-existing `Settings/astoria.ini` entry that briefly caused MFF to load as two separate DLL instances simultaneously after the move. Live-verified twice (Form Designer render + Toolbox populate) via `Compile.bat`.
- [x] **Toolbox component picker removed** — owner decision: no more choosing which `Controls\*` libraries appear in the toolbox; everything with a valid `Settings.ini` is always loaded. Deleted the "Add Components" toolbar button and `src/frmComponents.frm`; `LoadToolBox` always does a fresh folder scan instead of trusting stale persisted `Settings/astoria.ini` state. Found and fixed a real bug along the way: `GetFullPath` never collapsed a `..` segment unless it was at the very end of the path, silently breaking the toolbox's library-matching logic for every library except MyFbFramework (whose apparent match was coincidental, not correct). Live-verified: all 5 discovered libraries show at plain startup.
- [x] **`Controls/MyFbFramework/` renamed to `Controls/Framework/`** (owner request), with `MyFbFramework.wiki`/`MyFbFramework.vfp` renamed to match. Touched ~140 files: build scripts, core `src/` path references, `Controls/Framework/Settings.ini`'s own now-self-referencing keys, `LoadToolBox`'s folder-name-based MFF-load-order check, the IDE's own `.vfp`/`.code-workspace` files, all Example/Template project `ControlLibrary=` references (bulk replace, ~80 files), and the `Examples/MyFbFramework Examples` redirect file. Left alone: real upstream URLs to the actual `MyFbFramework` project, historical docs, prose mentions of the library's name, and pre-existing already-broken dev tooling. Live-verified: Form Designer renders, Toolbox shows all 5 libraries.
- [x] **MFF DLL renamed again — `astoria.dll` → `framework.dll`, moved back inside `Controls/Framework/`** — owner request, for consistency with the folder rename. Reverted the root-relocation from the earlier `astoria.dll` rename; since the DLL and its headers/sources share a folder again, `Settings.ini`'s path keys reverted to their original plain-relative forms (no more `../../` trick). Build scripts, `Main.bas`'s startup load, `IsMyFbFrameworkLibrary` (folder+filename check restored), and the two `Examples/Add-In` samples all updated. Live-verified: Form Designer renders, Toolbox shows all 5 libraries.
- [x] **`src/` indentation and line endings standardized (T01, partial)** — three files (`Main.bas`, `TabWindow.bas`, `frmOptions.frm`) had space or garbled mixed-tab/space indentation against the codebase's otherwise-universal tab convention; fixed by re-deriving correct nesting depth from surrounding `For`/`If` structure. All `src/*.bas`/`.bi`/`.frm` normalized to CRLF line endings, with scoped `.gitattributes` rules (`-crlf`) added so the conversion actually survives a commit — `core.autocrlf=true` silently reverts a plain CRLF conversion back to LF on `git add` otherwise. `Controls/` and `Examples/` still need the same pass (`2f445e4`).
- [x] **Toolbar tooltips added (T16)** — `cboBuildConfiguration`, the four Designer search boxes (Explorer/Toolbox/Properties/Events), and the code editor's `cboClass`/`cboFunction` dropdowns had no hint text; added, following the existing `.Hint = (...)` convention (`6f933f4`).
- [x] **Tools menu flattened** — removed the "Advanced" submenu; Add-Ins and External Tools are now top-level Tools menu items (`6f933f4`).
- [x] **Other Editors panel removed from Options** — owner decision: External Tools already covers registering a per-extension launch tool, making the dedicated panel in Tools > Options > Code Editor redundant. Removed entirely rather than just hidden: the panel, list view, Add/Change/Remove/Clear buttons, tree node, and load/save/ini wiring in `frmOptions.frm`/`.bi`; the `OtherEditors` dictionary, the extension-based double-click auto-launch routing, and `RunOtherEditorTool` in `Main.bas`/`.bi`; and the ini-load logic in `SettingsService.bas`. `frmPath.frm` (the shared Add/Change dialog also used by Terminal Paths and Help Paths) is untouched (`2f9d513`).
