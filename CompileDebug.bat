...
REM Skips mff when sources are older than framework.dll. Overwrites release build outputs.
REM Env: VERBOSE=1, FORCE_MFF=1, SKIP_MFF=1, NOPAUSE=1.

call "%~dp0BuildCommon.bat"

if "%BUILD_MFF%"=="1" (
	echo [%time%] Building framework.dll ^(debug^)...
	cd /d "%MFF_DIR%"
	"%FBC64%" -b "mff.bi" "mff.rc" -dll -gen gcc -mt -g -exx -x "../framework.dll" %FBC_VERBOSE%
