...
'
' URL IP
Sub URL_RemoveDomain(ByVal URL As String, ByRef domain As String, ByRef path As String, ByRef port As Integer) ' Split into domain and path
	' ByRef as String ,ByRef as String ,ByRef
	Dim As String  tob,ltob
	Dim As Long  aa,bb
	tob = Trim(URL)
	ltob = LCase(tob)
	If Left(ltob, 7) = "http://" Then ' Remove scheme
		tob = Mid(tob, 8)
		port = 80
		bb = 1
	...
	Else
		...
		bb = 0
	End If
	If bb = 1 Or Len(domain) = 0 Then  ' Has domain or no original domain
		'
		aa = InStr(tob, "/")
		If aa = 0 Then
			domain = Trim(tob)
		...
		End If
	Else   ' "HTTP/1.0 30" redirect - use original domain
		tob = Trim(tob)
		If Left(tob, 1) = "/" Then
			path = Mid(tob, 2)
		Else
