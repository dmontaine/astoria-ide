...
'DictionaryItem
	Private Property DictionaryItem.Key ByRef As WString
		If FKey <> 0 Then Return *FKey Else Return WStr("")
	End Property
...
End Sub
	...
	Private Sub Dictionary.Sort(bMatchCase As Boolean = False, ileft As Integer = 0, iRight As Integer = 0)
		...
		If bMatchCase Then
			While (i <= j) '/*Control single pass search within current group
		...
		Else
			...
			Wend
		End If
		If j  > ileft Then This.Sort(bMatchCase, ileft, j) '*Finally sort left partition the same way*/
		If i  < iRight Then This.Sort(bMatchCase, i, iRight) ';/*Sort right partition the same way; repeats until i = j in each group
		If OnChange Then OnChange(This)
	End Sub
	...
	Private Sub Dictionary.SortKeys(MatchCase As Boolean = False, ileft As Integer = 0, iRight As Integer = 0)
		...
		Else
			...
			Wend
		End If
		If j > ileft Then This.SortKeys(MatchCase, ileft, j) '*Finally sort left partition the same way*/
		If i < iRight Then This.SortKeys(MatchCase, i, iRight) ';/*Sort right partition the same way; repeats until i = j in each group
...
Private Sub Dictionary.LoadFromFile(ByRef Filename As WString)
	Dim As Integer Fn = FreeFile_, Result = -1, FileSize
	Dim As WString Ptr pBuff = 0
	If Trim(Filename) = "" OrElse Dir(Filename) = "" Then Exit Sub
	Result = Open(Filename For Input Encoding "utf-8" As #Fn)
	If Result <> 0 Then Result = Open(Filename For Input As #Fn)
	If Result <> 0 Then Exit Sub
	This.Clear
	FileSize = LOF(Fn) + 1
	If FileSize > 1 Then WLet(pBuff, WInput(FileSize, #Fn))
	CloseFile_(Fn)
...
End Function
	...
	Private Property Dictionary.Text ByRef As WString
		If FItems.Count < 1 Then Return WStr("")
		WLet(FText, "")
		For i As Integer = 0 To FItems.Count - 1
