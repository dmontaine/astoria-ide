...
'RowBackground	Background color of rows in DataGrid.
'AlternationRowBackground	Alternating row background color.
...
'GridLinesVisibility	Whether grid lines are visible.
'VerticalGridLinesBrush	Brush for vertical grid lines between columns.
'HorizontalGrdiLinesBrush	Brush for horizontal grid lines between rows.
'Supports multiple data types: text, number, date, image, and boolean
'https://www.cnblogs.com/browser-yy/p/3295939.html   C# format strings (repost)
