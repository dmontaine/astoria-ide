...
Namespace My.Sys.Forms
	...
	End Sub
		...
		Private Function NewWindowRequestedEventArgs.GetURL() ByRef As WString
				...
				Return *pURL
			#else
				'' ASTORIA CHANGE (re-applied from 1635bc4 after the upstream refresh): a ByRef
				'' WString result cannot return a literal -- it would reference a temporary that
				'' dies with the call, and the control fails to build. Return a static instead.
