...
'' NOTE: the DLL loader, not WebView2LoaderStatic. Static linking would drop the runtime-DLL
'' dependency entirely and was tried first, but WebView2LoaderStatic.lib is MSVC-built and
'' references __security_check_cookie and __guard_dispatch_icall_fptr, which the bundled
'' MinGW/GNU ld toolchain does not provide. Stubbing them would silently disable Control Flow
'' Guard checks inside Microsoft's loader, so the DLL is the honest choice; Astoria copies it
'' beside the built program instead (see BuildService.CopyFrameworkRuntimeDlls).
...
'' ASTORIA CHANGE: guard the RPC target-version constants. FreeBASIC's own Windows
'' headers already define these when windows.bi has been included first, which every
'' Astoria program does, so an unguarded Const here is a duplicate-definition error.
#ifndef TARGET_IS_NT100_OR_LATER
Const TARGET_IS_NT100_OR_LATER = 0
#endif
#ifndef TARGET_IS_NT63_OR_LATER
Const TARGET_IS_NT63_OR_LATER = 0
#endif
#ifndef TARGET_IS_NT62_OR_LATER
Const TARGET_IS_NT62_OR_LATER = 0
#endif
#ifndef TARGET_IS_NT61_OR_LATER
Const TARGET_IS_NT61_OR_LATER = 0
#endif
#ifndef TARGET_IS_NT60_OR_LATER
...
#define RPC_VAR_ENTRY __cdecl
	...
	#ifndef __MIDL_DECLSPEC_DLLIMPORT
	#define __MIDL_DECLSPEC_DLLIMPORT
	#endif
	#ifndef __MIDL_DECLSPEC_DLLEXPORT
