...
切记：MyFbFramework框架目录下如果没有 mff64.dll 和 libmff64.dll.a，你将不能进行可视化窗体设计。本分支仅支持 64 位 Windows VisualFBEditor。你可以下载MyFbFramework框架源码并按照下面提供的方法进行编译。
...
> **说明：** 本分支仅支持 64 位 Windows。也可在项目根目录运行 `Compile.bat` 一并编译框架和 IDE。
