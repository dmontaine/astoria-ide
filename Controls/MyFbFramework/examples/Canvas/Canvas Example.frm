...
' #Region "Form"
	...
	Dim Shared As Point Ms                  ' Coordinates when mouse button was pressed
...
Private Sub Form1Type.PictureBK_Paint(ByRef Sender As Control, ByRef Canvas As My.Sys.Drawing.Canvas)
	...
	With Canvas
		...
		.DrawColor = clYellow
		.Line (-100, 0, 100, 0) ' Draw X axis
		.Line (0, 100, 0, -100) ' Draw Y axis
		.Circle (0, 0, 10) ' Draw red center point
		.MoveTo(0, 0)
		...
		.Ellipse(10, -10, 10, 10)
		.TextOut 0,  0, "(0,0)" , clGreen, -1 ' Origin coordinates
		.TextOut 90, 10, "X", clGreen, -1    ' Label X axis
		.TextOut 5, 95,  "Y", clGreen, -1     ' Label Y axis
		
		For i As Integer = 10 To 50 Step 4
			.SetPixel(i, 10, clRed) ' Draw pixel
		Next
		
		'
		.DrawWidth = 1 ' Set pen width
		.DrawStyle = PenStyle.psSolid
		.DrawColor = clYellow
		.Line(-10, -10, -100, -10)
