...
Namespace My.Sys.ComponentModel
	...
	Private Function Printer.SetPrinterOrientation2 (ByRef PrinterName As WString, ByVal nOrientation As Long) As Long
			...
			' Fill DEVMODE structure with current settings
			...
			pDevMode->dmFields = DM_ORIENTATION ' Set flag that orientation is changing
			...
			' Set printer with new parameters
	...
	Sub Printer.GetPrinterPaperSizes(ByRef PrinterName As WString)
		...
		Dim i As Integer
			...
			If paperCount <= 0 Then
				Print "Failed to get paper sizes."
				Exit Sub
			End If
			
			' Get paper size names
			paperNames = _Allocate(Len(WString) * CInt(64) * paperCount) ' 64 characters per name
			If paperNames = NULL Then
				Print "Error allocating memory for paper names."
			...
			For i = 0 To paperCount - 1
				...
				pPaperSize->Width = pSizes(i).x ' " tenths of a millimeter"
				pPaperSize->Height = pSizes(i).y ' " tenths of a millimeter"
			Next
