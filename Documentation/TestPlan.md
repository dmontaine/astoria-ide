...
| --- | --- | --- | --- | --- | --- |
| A1 | **SQLite3Component data path** — create a database, create a table, insert rows, query them back, verify the values, close. | The database data path works, not merely that it compiles. Closes the single largest known gap. | 🤖 | - | |
...
| A4 | **WebBrowser rendering** — navigate to a local HTML file with known text and a known colour, screenshot, confirm the page actually rendered. | Page rendering, the second-largest gap. A screenshot makes this agent-checkable. | 🤖 | - | |
| A5 | **WebBrowser navigation** — follow a link, go back, read the resulting URL/title. | Navigation and history, not just first paint. | 🤖 | - | |
