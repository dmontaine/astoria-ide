...
| C4 | **Multi-select operations** — select several controls, align/size/space them, undo, redo. | The Form Designer menu's core value, against multiple controls at once. | 🤝 | ⚠️ | **Align and size pass; undo/redo does not exist.** Owner scattered four labels, multi-selected them and applied Align Left then Make Same Size: all four ended at `Left = 190` and `75 x 25`, and the result persisted correctly through a save. Multi-select and the alignment commands work. **Ctrl+Z and Ctrl+Y did nothing**, and the cause is that the designer has no undo at all — `src/Designer.bas` contains no Undo/Redo implementation, and `Undo` is routed to the text editor (`AstoriaIDE.bas`: `Case \"Undo\": txt->Undo`) behind a check that the active control is a `TextBox`/`RichTextBox`. In Form view that is false, so the keystroke silently does nothing. This is **absent rather than broken**, and it is worse than it sounds: there is no error and no recovery. A misapplied align across several controls cannot be taken back — the user must reposition each control by hand. Recorded as ROADMAP §13.19 with a cheap honesty fix (grey Undo/Redo out in Form view, so the UI stops implying an action it will not perform) and the real fix (a designer undo stack) costed separately. |
