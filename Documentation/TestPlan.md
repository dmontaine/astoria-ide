...
| C2 | **Designer round-trip fidelity** — save, close, reopen the project; confirm every control, property and handler survived. | `.frm` generation and re-parsing, the file format the whole product depends on. | 🤝 | ⚠️ | **Partly established, and reclassified from 🤖 to 🤝.** What is now known: a `.frm` is saved as the *editor's text buffer* (`TabWindow.SaveTab` → `txtCode.SaveToFile`), not serialised from a designer object model — so opening a form and saving it unmodified is lossless by construction, and was confirmed to be a **no-op**: the IDE correctly does not rewrite an unmodified file (byte-identical, timestamp unchanged). That means the real risk is narrower than the scenario first assumed: it is not "does saving mangle the file" but "does a **designer edit** regenerate its region without disturbing the rest". Reaching that automatically needs the form switched to Form view and a control selected on the designer surface; the project opens in code view and the designer surface is not reachable by class name, so this needs a person. **Owner step (about 30 seconds):** open a form in Form view, nudge a control with an arrow key, Ctrl+S — then the file can be diffed to confirm the control's new bounds landed and nothing else moved. Finding along the way: chasing the designer's key handling exposed 14 more instances of the modifier-key bug B2 found (see B2), this time in Astoria's own `Designer.bas`, `EditControl.bas`, `Main.bas` and `TabWindow.bas` — so designer Ctrl/Shift operations were affected. Fixed. |
