...
| A4 | **WebBrowser rendering** — navigate to a local HTML file with known text and a known colour, screenshot, confirm the page actually rendered. | Page rendering, the second-largest gap. A screenshot makes this agent-checkable. | 🤖 | ❌ | **The control cannot render, and using it crashes the program.** Screenshots show a blank white pane; `Navigate()` never returns, and from a button click the process dies. Root cause indicated: the control is hosted via ATL `AtlAxWin`, which selects the ActiveX control to create from its host window's **text** — but the host window is created with empty text (confirmed by enumerating the child: `[AtlAxWin]` with no caption), even when `.Text = "about:blank"` is set as the framework's own example does. Nothing is hosted, so the `IWebBrowser2` pointers stay null and `Navigate` dereferences a null vtable. Verified from both a timer callback and a button click, so calling context is not the cause. Run: `Examples/Integration/A4_WebBrowserRender`. **Awaiting owner decision on the fix** (see note below). |
| A5 | **WebBrowser navigation** — follow a link, go back, read the resulting URL/title. | Navigation and history, not just first paint. | 🤖 | ⏸️ | Blocked by A4 — there is no rendered page to navigate from. `a4_page2.html` and the link are already in place for when A4 is fixed. |
...
| ID | Scenario | What it proves | Who | Status | Result |
| --- | --- | --- | --- | --- | --- |
| B1 | **Data-entry form** — Label + TextBox + ComboBox + CheckBox + RadioButton group + CommandButton. Fill fields, click the button, have the handler read every value and print them. | The most common form shape in existence. Proves event wiring and cross-control value reads. | 🤖 | - | |
...
| B4 | **Docking and anchoring under resize** — mixed `alTop`/`alLeft`/`alClient`/`alBottom` children; resize the window large, small, and maximised; screenshot each. | Layout maths under real resizing. Astoria's own Options dialog has produced several such bugs. | 🤖 | - | |
...
| B6 | **List/detail** — ListView or TreeView selection driving a detail panel of TextBoxes. | Selection events propagating between controls; the second-most-common form shape. | 🤖 | - | |
...
| B10 | **Second form** — main form opens a modal dialog and a modeless window, passes data both ways, closes them. | Multi-form lifetime and ownership — the area that produced the modal z-order defect in the IDE itself. | 🤖 | - | |
