...
| C2 | **Designer round-trip fidelity** — save, close, reopen the project; confirm every control, property and handler survived. | `.frm` generation and re-parsing, the file format the whole product depends on. | 🤝 | ✅ | **Passes after fixing a real defect it found.** Owner made a designer edit (nudged a control) on a UTF-8-with-BOM form and saved; the file was then diffed byte for byte. **First run: three differences** — the intended move, an orphaned `#include` from a control that had been placed and deleted, and **the UTF-8 BOM silently stripped**. The BOM loss was traced to `TabWindow.bas`: `LoadFromFile` takes the encoding and line-ending style **ByRef** and reports what it detected, and the next two lines discarded that with `FileEncoding = Utf8` / `NewLineType = WindowsCRLF`. So every edited file was being rewritten as BOM-less UTF-8 with CRLF whatever it had been — not cosmetic in FreeBASIC, where a BOM changes how string literals compile, and it would rewrite every line of an LF file. Fixed by letting the detected values stand: changing a file's format is an explicit action, never a side effect of saving. **After the fix: exactly one difference** — the `SetBounds` line for the control that was moved. Every other control, property, handler, comment and include byte-identical; BOM and CRLF preserved. That is the round-trip fidelity this scenario exists to prove. **Observation, deliberately not "fixed":** deleting a control leaves its `#include` behind. That is defensible — auto-removing an include could break hand-written code elsewhere in the file that uses the type — so it is recorded rather than changed. |
