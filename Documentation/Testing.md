...
ten E11 checks pass, D1 was re-run as a startup regression check, and the fix is **owner-verified by
hand** (2026-07-19): launching Astoria while it is already running brings the running IDE forward.
...
| **Accessibility and display scaling** | **Display scaling partly closed 2026-07-19 (E8):** the IDE passes visual use at 125%, 150% and 200% on one monitor. Mixed-DPI monitor movement was unavailable. **High contrast tested and now passing 2026-07-19 (E10a), owner-verified.** Testing found that Astoria never detected high contrast at all: the chrome survived because it paints with system colours, but the editor took its colours from a theme file, so under a *light* Astoria theme the editor stayed light inside a dark high-contrast IDE and **line numbers vanished**. Fixed (ROADMAP §13.30): the editor now takes the system background under high contrast and keeps a theme colour only where it clears a 4.5:1 contrast ratio, so syntax highlighting survives where it is readable and is replaced where it is not. **Known and accepted:** changing the Windows theme *while Astoria is running* leaves colours mixed until it is restarted — colours resolve at load. The owner judged this acceptable for 1.0. **Screen readers remain untested and are not testable here** (E10b): that needs someone who uses one. **Keyboard-only operation: the blocker is fixed as of 2026-07-20, one step still fails.** The New Project dialog — which the IDE opens into when no project is loaded — now takes an initial focus, moves focus on Tab, and closes on Escape; it previously accepted no keyboard input at all and could only be dismissed with Alt+F4. The cause was a single framework omission (`ShowModal` did not focus a control the way `Show` does), and the fix changes behaviour for user programs too, not just the IDE. **Still failing:** the project tree cannot be reached from the keyboard, so a mouse-free user still cannot open a file. See ROADMAP §13.28. Original finding: **Keyboard-only operation tested 2026-07-19 (E9) and it FAILED** — this is now a known defect rather than an untested gap. Building and running are reachable from the keyboard, and the menu bar is properly keyboard-driven, but **a project cannot be created or edited without a mouse**: the New Project dialog accepts no keyboard input at all (not even Escape to close it), and the project tree cannot be reached, so no source file can be opened. See ROADMAP §13.28. Screen readers and high-contrast mode remain untested (E10). |
