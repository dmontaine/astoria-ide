...
| **Controls used together** | **Started 2026-07-18** — four multi-control scenarios pass: a data-entry form reading six controls back (including a radio group that correctly deselects), docking under resize (50/50 checks across five window sizes), list/detail selection driving detail fields, and a second form covering modal round-trip and modeless coexistence. That is the first real coverage of controls cooperating rather than each opening alone. Nine more scenarios remain unrun — nested containers, shared ImageList, menu/toolbar/statusbar together, timer-driven updates, database-to-view, and a twenty-control density check. See [TestPlan.md](TestPlan.md) § B. |
