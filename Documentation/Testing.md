...
| **WebBrowser rendering** | **Tested 2026-07-18 and it FAILS.** The control does not render a page, and calling `Navigate` crashes the program. It hosts its ActiveX control through ATL `AtlAxWin`, which picks the control to create from its host window's text — and that window is created with empty text, so nothing is hosted and the interface pointers stay null. This is why WebBrowser was the one control the 73-control sweep could never get an owner visual confirmation for: its window opens, but it is an empty host. See [TestPlan.md](TestPlan.md) A4 for the evidence and `Examples/Integration/A4_WebBrowserRender` to reproduce. Awaiting a decision on the fix. |
