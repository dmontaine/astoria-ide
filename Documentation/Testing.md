...
| **Controls used together** | **Started 2026-07-18** — nine multi-control scenarios pass: a data-entry form reading six controls back (including a radio group that correctly deselects), docking under resize (50/50 checks across five window sizes), list/detail selection driving detail fields, a second form covering modal round-trip and modeless coexistence, and one ImageList shared by a ToolBar, TreeView and ListView — which confirmed that the ordering trap behind the dead menu icons applies to those controls too; three levels of container nesting where every child's window handle survives a tab switch; a density check putting 26 different control types on one form; database query results reaching a ListView with ordering intact; and a browser driven by an address box, a Go button and a status label. That is the first real coverage of controls cooperating rather than each opening alone. Four more scenarios remain unrun — nested containers, shared ImageList, menu/toolbar/statusbar together, timer-driven updates, database-to-view, and a twenty-control density check. See [TestPlan.md](TestPlan.md) § B. |
