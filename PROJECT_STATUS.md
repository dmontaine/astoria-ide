...
>
> **⏭️ CROSS-MACHINE HANDOFF (2026-07-12, Intel machine → other computer).** **`git pull` first thing** — local and `origin/main` are in sync at `63104f1` as of this handoff, nothing uncommitted except the two pre-existing not-ours files (`Controls/MyFbFramework/mff/Temp.bas`, `Settings/astoria.ini` — leave alone) and local-only trace logs. **Done this session (Intel machine, Opus then Sonnet):** **DR-4 FIXED + owner-verified** (gutter-click horizontal-scroll jump — see table); **"2-clicks-to-close" investigated, confirmed NOT a code bug** (Windows activate-then-act on an unfocused window); **DR-16(b) FIXED** (`deinit`'s worker-thread UI touches now marshal via `bDeinitCleanupPending`/`FlushDebugOutputOnUI`, DR-7 pattern) — all release-rebuilt + committed, tracked `astoria.exe` is current. **Next up, in order: (1) DR-16(a)** — `load_file`'s blocking modal `MsgBox` on the worker thread. This needs an **owner design decision first**, not a mechanical fix: does closing the IDE mid-`load_file`-error dialog still need to block on the MsgBox, or can it be deferred/non-blocking? Decide, then the fix follows the same stage-then-flush pattern as DR-16(b). **(2)** Phase 4 residual — two dead-code items found during the DR-16 audit, not yet removed: `kill_debug()` (zero real callers, orphaned since DR-6/2B) and `line_highlight`'s legacy `"[Inferior 1"` branch (unreachable — `run_debug`'s loop intercepts that output upstream now). Small, low-risk, mechanical. **(3)** The instrumentation-strip pass (`bDbgTrace`/remaining `DbgTrace` calls) once the sub-project's behavioral work is judged done. **⚑ Gotchas:** (1) **rebuild + commit a RELEASE exe (`Compile.bat`) after every debugger-behavior source fix, immediately** — a prior session lost time when debug-build verification runs left the tracked exe stale relative to source; every fix this session was release-rebuilt before committing, keep doing that. (2) `UseDebugger=false` in `astoria.ini` is STALE (written only on clean exit) — the live Run ▸ Use Debugger toggle is what counts. (3) A program with **no bound breakpoint runs to exit** (DR-8 removed break-at-entry); breakpoints on **comment lines** don't bind. (4) Trace logs are git-ignored/local-only — each machine accumulates its own. (5) Source files have MIXED line endings (LF-only lines inside CRLF files) — use byte-precise single-line `Edit` anchors, never large multiline string matches. (6) **NEW: if Claude launches `astoria.exe` for you via a script/automation, click the window once before testing anything** — Windows blocks a background-launched process from getting true foreground focus, and an unfocused window can produce misleading "regression" symptoms (this session chased a false DR-4/close-bug "reversion" for a while before finding this — see the status-at-a-glance entry above for the full story).
