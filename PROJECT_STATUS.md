...
>
> **⏭️ CROSS-MACHINE HANDOFF (2026-07-11, mid-session — switching machines):** **The committed `astoria.exe` carries BOTH sets of instrumentation**: (1) the original GDB `DbgTrace` (gated `bDbgTrace=True` in `Debug.bas:362`, writes to `Settings\debug_trace.log`) — still live, used for all debugger repros; (2) **NEW** `DbgTrace` markers through `CloseProject`'s teardown + `tvExplorer_SelChange` in `src/Main.bas` (9 markers: `CloseProj.enter/tabsClosed/safetyPassed/nodesFreed/beforeNodesRemove/afterNodesRemove/clearAnalysis/clearDebug/changeUseDbg/changeMenus/done`, `SelChange.enter/beforeOpen/afterOpen`) — added for **DR-11**, **committed as temporary instrumentation** (same pattern as the original trace build) so it survives this machine switch; strip both sets once their diagnoses are done. **Update 2026-07-11 (single machine now): DR-3 (both modes) + DR-13 fixed via slices 2A+2D and owner-verified; DR-11 re-tested and RESOLVED (downstream of DR-3, no repro).** The DR-11 `CloseProject`/`SelChange` markers can be stripped (DR-11 closed); the GDB `DbgTrace` set stays until DR-6 is exercised. **Next fragile-core work: DR-6 (slice 2B)** — needs a live Stop-**while-running-freely** repro (both earlier Stop tests took the safe `q` path); then **DR-1/DR-10 (slice 2C)** — route the F9/gutter/menu toggle paths to arm during debug (checklist: hit-bp / comment-line-refusal / toggle-off). Lower priority: DR-7 residuals (Output `ShowMessages` + watch-edit `UpdateWatch` on the worker), DR-8 (break-at-entry UX), confirm DR-2 stays resolved. Then **Phase 4**: strip both trace instrumentations + dead-code sweep. **Trace logs (`Settings\debug_trace.log`, `*.pass1.log`, `*.pass2.log`) are local-only, NOT git-synced** — each machine accumulates its own; archive/rename before a fresh repro to keep captures separable. **Note: source files have MIXED line endings (LF-only lines inside CRLF files) — use byte-precise edits, not exact multiline string matches, for any further Phase 2 work.**
