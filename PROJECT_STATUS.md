...
>
> **⏭️ CROSS-MACHINE HANDOFF (2026-07-12, Intel machine → other computer). `git pull` first thing** — synced to `origin/main` @ `c906d19`, nothing uncommitted but the usual not-ours noise (`Controls/MyFbFramework/mff/Temp.bas`, `Settings/astoria.ini`, `Examples/DeviceExplorer/DeviceExplorer.vfp` — leave alone). **This session:** (1) DR-16(a) done + owner-verified — **the Debugger Reliability sub-project (DR-1…DR-16) is now fully closed**; (2) Phase 4 dead-code items removed (`90f4dc7`+`64ea0ed`); (3) **first review of `Controls/MyFbFramework/` (MFF, the vendored GUI framework) — see the new [MyFbFramework review](#myfbframework-review-fable-2026-07-12) Open-Items subsection + the full report.** **⚠️ REPORT IS LOCAL-ONLY:** `Documents/mff_review.md` lives *outside the git repo* (same as `fable_review.md` / `debugger_fragility_audit.md`), so a `git pull` on the other computer will **not** bring it — the actionable task list is embedded inline in Open Items (self-contained: each task has file:line + fix), but if you want the full findings/framing, **copy `Documents/mff_review.md` to the other machine manually.** **MFF next steps:** start with **T0** (pin the upstream baseline; decide patch-vs-upstream-vs-drop for the networking controls — may delete half the task list), then **T-OPUS-1** is the highest-value fix (the `ThreadsEnter` no-op stub = root cause of the DR-3/DR-7 class, helps the IDE itself). **Owner asked whether a deeper MFF review is warranted — Fable's answer: yes but scoped, not exhaustive.** Do T0 first (dropping unused controls shrinks the surface for free), then a *targeted* Opus deep-pass on the ~6–8 hot-path core files only — `Control.bas`, `Form.bas`, `Canvas.bas`, the `WndProc`/message dispatch, and the GDI create/select/delete/restore lifecycle across `Canvas`/`Bitmap` (where an undiscovered bug would hurt the IDE itself, and where the unconfirmed GDI-handle-leak suspicion F-R-gdi lives). That's a bounded ~half-session effort, not a 200-file audit. **Remember: any MFF source edit needs a `mff64.dll` rebuild (`FORCE_MFF=1`) to take effect.**
>
...
*First review of `Controls/MyFbFramework/` (the vendored, now-git-tracked LGPL UI framework the IDE is built on). Full report: [`Documents/mff_review.md`](../../Documents/mff_review.md). MFF plays two roles — the IDE's own runtime dependency (`mff64.dll`) **and** a control library shipped to IDE users; severity depends on which (see report §0). **Any MFF source edit needs a `mff64.dll` rebuild (`FORCE_MFF=1`) to take effect.** All tasks gated on **T0**.*

- [ ] **T0 (owner decision + `UPSTREAM.md`)** — record the upstream MFF baseline commit (there's currently no pin, so fork-local edits can't be told from inherited code), and decide the posture for the networking controls: (a) patch locally, (b) upstream the fixes, or (c) **drop `HTTPServer`/`HTTP` from the shipped set** if no AstoriaIDE user needs them — option (c) deletes the entire F-N* finding class at once. Everything below is gated on this. **(Owner / Fable)**
- [ ] **T-OPUS-1 — resolve the `ThreadsEnter`/`ThreadsLeave` contract (F-M1, highest-value fix).** `Component.bas:257-261` — these are **no-op stubs**, so every `ThreadsEnter…ThreadsLeave` block framework-wide (and in `src/`) is an unsynchronized race. **This is the framework-level root cause of the IDE's DR-3/DR-7 debugger-hang class.** Either implement them as a real process-wide critical section, or formally document them as no-ops and fix `THREADING.md` (which currently claims they work). Helps the IDE directly, not just user apps. **(Opus)**
- [ ] **T-OPUS-2 — audit `UString.AppendBuffer`/`Resize` accounting (F-M2).** `UString.bas:361-377` mixes char-count and byte-count units with opaque capacity bookkeeping; `UString` is the most-used type in the codebase (huge blast radius). Boundary unit tests; fix or prove-correct. **(Opus)**
- [ ] **T-OPUS-3 — `HTTPServer` memory-safety + traversal hardening (F-N1/N3/N4).** *(Gated on T0 = patch/upstream.)* Fix the `ReceivedBuffer` leak on early disconnect, the not-nulled-after-free dangling pointer (a latent remote UAF currently masked only by broken keep-alive), and the broken percent-decoder (`c-48` treats hex A–F as decimal) + replace the fragile `InStr("..")` traversal filter with real path canonicalization vs. `HomeDir`. Review the combined diff. **(Opus)**
- [ ] **T-SON-1 — `HTTPServer` DoS caps + safe bind (F-N2/N5).** *(Gated on T0.)* Configurable max request size (body + pre-header accumulation — currently unbounded via trusted `Content-Length`); default bind to `127.0.0.1` instead of `INADDR_ANY` (user servers are currently LAN-exposed by default) + expose the bind address. **(Sonnet)**
- [ ] **T-SON-2 — `HTTPConnection` hardening (F-N7).** *(Gated on T0.)* Optional response-size cap; make the hardcoded spoofed Chrome User-Agent a property with a neutral default; document the blocking 3×`Sleep(1000)` retry. **(Sonnet)**
- [ ] **T-SON-3 — unchecked-`Open` sweep in MFF (F-R-mff).** Same class as the main review's F-R2: apply check-result-and-bail to `CheckedListBox.bas:272,289`, `ComboBoxEdit.bas:569,586`, `ListControl.bas:448,465`, `Bitmap.bas:154`, `Animate.bas:335`. **(Sonnet)**
- [ ] **T-SON-4 — `Registry` robustness (F-R-reg).** `Registry.bas` silently truncates values > 2048 bytes (fixed buffer); `WriteRegistry` does no input validation. Dynamic-size query + validate inputs. **(Sonnet)**
- [ ] **T-HAIKU-1 — remove debug `Print` in `HTTPServer.bas:165` (F-N6).** *(Gated on T0 ≠ drop.)* One-line; every request currently prints to the user program's stdout. **(Haiku)**
- [ ] **T-DEFER (MFF) — not blocking:** dead-control trimming (~150 unused files compiled into `mff64.dll`, F-H3), CN/example baggage (F-H2), `Canvas` GDI-handle audit if long-session handle growth is ever observed (F-R-gdi). Revisit only with owner appetite or an observed symptom.
