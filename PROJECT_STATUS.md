...
**Active work:** Opus's "Next Steps" review (`Next Steps - Opus.md`, 2026-07-07) evaluated three prior AI reviews against the actual source and produced a verified, sequenced backlog. **Every phase is now done and committed except R5 (bounded GDB read) and E1 (Apply dirty-tracking)**, both deliberately spun off rather than risked inline (see below). 13.3.A S1–S7 are **all committed** (Opus-reviewed). A Run-toolbar persistence regression the owner found post-S4 (my own S3 migration latching Run permanently visible) was root-caused and fixed 2026-07-07 — see the S5–S7 review outcome below. Next: R5 (needs a dedicated live-GDB session), E1 (already running as a background task), or resume §13.3 step-by-step UI review with the **View menu** (File + Edit complete; Search → Define reliability pass shipped with S1–S4).  
**Open items consolidated:** see [Open Items](#open-items) below.

---

## Opus "Next Steps" backlog — all phases done except R5/E1 (2026-07-07)

Executes Phase A, item 5 (P1), Phase B (P2/P3), Phase C (U1/U2, U3/U4, event-handler cluster, `.lng` decision), Phase D (F1, F2), and Phase E (O1, O2, O3) of `Next Steps - Opus.md`'s consolidated, source-verified backlog (Opus cross-checked three prior AI reviews — Cursor, Deepseek, Sonnet — against the actual code before sequencing this plan; see that document for full methodology and the discarded/unsubstantiated claims). Only R5 (needs live GDB reproduction) and E1 (large rewrite, spun off as its own task) remain.
...
- **E1** — `cmdApply_Click`'s ~400 unconditional INI writes (96 distinct `WriteXxx` call sites, many in loops over AI agents/terminals/editors/helps/build configs/include+library paths): **owner wants this fixed**, but it's a large, delicate rewrite of a ~400-line Sub, not a quick win — spun off as its own background task (`task_ea2834b6`, "Add dirty-tracking to Options Apply INI writes"), **now running in a separate session**.

**Remaining Opus backlog:** R5 (GDB read timeout — needs a dedicated live-GDB session, see above), E1 (running as background task `task_ea2834b6`), and Phase F (deferred structural: file splits, `FormDesign` debounce — deliberately last, touches the fragile designer path; not started, low priority, see `Next Steps - Opus.md`). Everything else in the Opus backlog is done.
...
**Known D1 gap (owner-reported 2026-07-07): menu does not grey on form/tab close.** `tabCode_SelChange` has early-exits (`If tb = tbOld Then Exit Sub`, Main.bas ~8156/8170) that return *before* the enable-logic, so closing a form that leaves the same tab "re-selected" skips the update and the menu stays enabled. Fix pending: move the Designer-menu enable/disable to a spot that always runs on document change (or hook the close path). Not yet done.
...
**Still open (carried forward):** the **D1 form-close greying gap** above (Designer menu doesn't grey when the form/tab closes, due to the `tabCode_SelChange` early-exits) — deferred to next session.
...
- **Sonnet (mechanical / contained):** **E1** (Apply dirty-tracking — *already running*, `task_ea2834b6`); **C1** Comment→Toggle-Comment merge; **C2** move Bubble Help/Suggest Options/Parameter Info into Options; **C3** move Recent AI Chat into AI Agent panel; **C4** `.lng` Options simplification (decision made: English-only, so now just the low-risk hide/relabel pattern); **C5** GitHub submenu reduction (after owner names the 2 items); **B1** `DeleteEditorFile` `.vfp` dirty-sync; **B2** `frmNewProject` template icons; **B3** `OpenRecentFiles()` dialog; **O1/O2** Terminal / Other Editors removal *only if owner opts to remove* (default: leave — working features, harmless).
