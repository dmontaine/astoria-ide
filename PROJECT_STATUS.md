...
- [ ] **H-1 (M, D2D path only — IDE force-disables D2D) — `Canvas.Cls` leaks an `HBRUSH` on the Direct2D early-return (`Canvas.bas:257,264,303`).** Brush created unconditionally, `Return` before `DeleteObject B` → per-`Cls` GDI leak in a user app that enables D2D. Fix: create/delete `B` only where used. **(Sonnet)**
