...
> ⏸️ **DEBUGGER WORK EXTRACTED INTO ITS OWN SUB-PROJECT (2026-07-11, WIP — synced across machines via git).** T6 began as one Fable-remediation task (the `set_bp` pipe race). Owner live-testing of the attempt-#2 fix surfaced **three more distinct, separately-rooted debugger defects**, so debugger work is now a dedicated reliability effort — see [⭐ ACTIVE SUB-PROJECT — Debugger Reliability](#-active-sub-project--debugger-reliability-queued-2026-07-11) for the full charter, defect table (DR-1..DR-5), and plan. **Fable-remediation Wave 4 is now: T5 done (owner-verified); T6 + all further debugger work moved to that sub-project.**
>
> Known defects (detail in the sub-project): **DR-1** `set_bp` pipe race (former T6 — attempt #2 code compiles clean, owner-verify *incomplete*, interrupted by the others); **DR-2** Step-Into no-highlight/no-scroll; **DR-3** rapid-F8 hang; **DR-4** gutter-click blanks the editor until refocus (new; reproduces *only while debugging* — owner-confirmed — the gutter path never touches GDB, so the culprit is `PaintControl` rendering incompletely during an active debug session; likely the same paint root cause as DR-2). DR-2/3/4 are **not** caused by the `set_bp` change (it only touches the F9 path).
>
> **Standing rule (carried over from the T6 charter): instrument-first — add file-trace, have the owner reproduce, read the actual interleaving, *then* fix. Do NOT reorder the GDB worker loop without a confirming trace** (attempt #1 hard-locked doing exactly that). The full attempt-#1/#2 write-up remains in the "Breakpoint-during-debug pipe race" Open Item.

**Last updated:** 2026-07-11 — **Fable review remediation nearly complete.** Wave 1 (T8–T13, hygiene) done; T1 decision gate resolved (owner chose **Option B — per-user install**, portable model kept, no APPDATA migration); Wave 3 (T3 PipeCmd rework, T4 native project delete, T7 unchecked writes, T2a/T2b portable-model safety net, T17 English-only sweep) all done and committed individually; T16 (Fable adversarial review) done with all 3 findings fixed; T5 (quote the bundled GDB launch path) done and owner-verified against a spaced path. **The Fable review remediation sub-project is CLOSED (2026-07-11):** all tasks T1–T17 done (T15 memo delivered, owner chose Option B, 10-theme shortlist implemented and committed `9c18bd8`), and the accumulated owner smoke-test list completed (owner-confirmed 2026-07-11 — see the T16 row for the findings, all already dispositioned). The debugger work (formerly T6) continues separately in the [Debugger Reliability sub-project](#-active-sub-project--debugger-reliability-queued-2026-07-11) (DR-1..DR-5); the T14 archival of the remediation section to HISTORY.md is now unblocked. An owner-found **"Add" dialog bug** (shared `frmPath`'s `Form_Close` clobbered `ModalResult` on every close, silently breaking every Add flow) was root-caused and fixed the same day. Separately, the debugger **Step Out** menu item was promoted to the top-level Run menu with clarifying step tooltips (2026-07-11). Full dated session-by-session narrative is archived in [HISTORY.md](HISTORY.md).
...
|---|---|---|---|---|
| **DR-1** | `set_bp` pipe race (was **T6**) | F9 breakpoint mid-session can race the worker thread on the shared pipe; also plants phantom GDB breakpoints on comment lines | attempt #2 code written, compile-clean, **owner-verify incomplete** (interrupted by DR-2/3). Full attempt-#1 (reverted) + attempt-#2 history in the Open Item. | `Debug.bas` `set_bp` (~1791); shared `hReadPipe`/`hWritePipe` (`:382`); worker loop (~2024+) |
...
| **DR-3** | Rapid-F8 hang (**BUG B**) | Mashed/held Step Into freezes the app | root-caused **statically**: worker dequeues + sends a new command while `Running=True` (`Debug.bas:2056`–2069), so back-to-back `s` sends desync the read stream — `RefreshDebugPanelsAfterStop` (`:1986`) reads leftover step annotations instead of its own responses → a later `readpipe` blocks forever | planned fix: reorder worker loop to **strict lockstep** (read pending stop → refresh panels → *then* dequeue/send next). **Needs trace confirmation before touching the loop.** Verified safe vs. the End-while-running path (`q\n` only enqueued when not running). |
| **DR-4** | Gutter-click blanks the editor (**BUG C**, new 2026-07-11) | Clicking the gutter to toggle a breakpoint makes most displayed lines disappear until you click back into the code window — **only while the debugger is active** (owner-confirmed 2026-07-11: no repro with the debugger off) | code path traced 2026-07-11; **debug-mode paint pipeline, not `set_bp`** | Path verified: gutter-click → `WM_LBUTTONUP` (`EditControl.bas:6990`) → `EditControl.Breakpoint` (`:233`) → toggle flag + `PaintControl`. It **never** calls `set_bp` or the parent `Case "Breakpoint"` — so no GDB involvement (rules out DR-1). `EditControl.Breakpoint` runs the *same* code debugging or not, so the difference is that **`PaintControl` renders incompletely while a debug session is active** — the paint path branches on the executed-line state (`CurExecutedLine`/`OldExecutedLine`/`CurEC`, `EditControl.bas:3227/4366/5030`), only populated during debug. **Strong hypothesis: same root cause as DR-2** (debug-mode paint pipeline) — investigate together. "Click in the code window recovers it" = a focus/caret event forcing a fuller repaint. |
...
### Plan (instrument-first)

- **Phase 1 — one trace pass.** Add focused file-trace covering: worker dequeue + `Running` state, every write/read result, `line_highlight` parse outcome + resulting `fcurlig`, `TimerProcGDB` action per tick, and the `EditControl.Breakpoint` gutter-toggle paint path (DR-4). Owner reproduces DR-2 (single step), DR-3 (mash F8), DR-4 (gutter-click), and the DR-1 checklist. Read the interleaving before changing anything.
- **Phase 2 — fix in dependency order.** DR-3's worker-loop lockstep reorder is foundational (DR-1's `set_bp` and rapid-step both depend on a correctly-synced loop), so likely: confirm + fix DR-3 → re-verify DR-1 on the fixed loop → **DR-2 + DR-4 together** (shared debug-mode paint pipeline) per what the trace shows. Each fix: smallest diff, owner live-verify, then commit.
- **Phase 3 — full re-verify.** Owner runs the complete debugger checklist (set bp → step → inspect → continue → stop; F9 mid-session; gutter-click; rapid step) on the final build before the sub-project closes.
