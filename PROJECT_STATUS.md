...
- **Phase 1 — one trace pass.** Add focused file-trace covering: worker dequeue + `Running` state, every write/read result, `line_highlight` parse outcome + resulting `fcurlig`, `TimerProcGDB` action per tick, and the `EditControl.Breakpoint` gutter-toggle paint path (DR-4). **Add (per Phase 0): thread-id on every pipe I/O + `ShowMessages`/`DoEvents` call (confirms DR-7), and `kill_debug`/`deinit` entry + `tlockGDB` owner (confirms DR-6).** Owner reproduces DR-2 (single step), DR-3 (mash F8), DR-4 (gutter-click), the DR-1 checklist, **and a Stop-mid-session repro for DR-6**. Read the interleaving before changing anything.
