...
| 12 | 3 — robustness | **T16 — Adversarial review of Wave 3** | Fable | — | Owner changed Wave 3 to per-task commits (2026-07-11) — review T2+T3+T4+T7 (they jointly touch every external-process launch and every settings write) as a set of individual commits rather than one combined diff: injection, paths-with-spaces, failure paths, clipboard, threading. Findings go back to Sonnet as follow-up commits. Owner smoke test covers the accumulated set: Open Containing Folder, an external tool from Tools ▸ External Tools, Delete Project (on a disposable project), Options ▸ Apply, a clipboard-preservation check (copy text → do the above → paste), and double-clicking a file with a registered External Editor (T3's UI-freeze fix) |
