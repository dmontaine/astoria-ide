...
| 8 | 3 — robustness | **T3 — `PipeCmd` rework** | Sonnet | F-S1, F-S5 | Remove the `\|clip` suffix (clipboard clobber) and the blanket `cmd /c` wrapper (`TabWindow.bas:10967-10971`); audit all call sites individually: `frmTools.frm:387,400`, `Main.bas:1240,2433,6905,8341` (+ `2815`, which T4 owns — coordinate, don't double-fix). `ShellExecuteW "open"` for Explorer/URLs (`OpenUrl`, `Main.bas:1236-1240` — URLs containing `&` currently break under `cmd`); direct `CreateProcess` for external tools unless the target is a `.bat`; keep `INFINITE` waits off the UI thread |
| 9 | 3 — robustness | **T4 — Native project delete** | Sonnet | F-S2 | Replace the shell `rd /s /q` (`Main.bas:2815`) with `SHFileOperationW` (error codes + recycle-bin option); surface failure via `MsgBox` per the F1 feedback-channel policy (irreversible destructive action = MsgBox tier). Preserve the `ChDir ExePath` guard and its rationale comment (`Main.bas:2808-2812`) |
| 10 | 3 — robustness | **T7 — Check the 6 remaining unchecked `Open For Output` writes** | Sonnet | F-R2 | Replicate the approved R2 pattern (check result → `MsgBox` + bail) at: `BuildService.bas:259`, `frmImageManager.frm:398`, `frmOptions.frm:3119`, `frmTools.frm:241`, `Main.bas:8326`, `TabWindow.bas:2884`. The two user-data sites (frmImageManager resource file, TabWindow generated `.bi`) matter most |
| 11 | 3 — robustness | **T2 — Portable-model safety net (redefined by T1 sign-off, 2026-07-11)** | Sonnet | F-S4 | **T2a:** startup writability probe — early in startup, verify `ExePath\Settings` is writable; if not, one clear MsgBox ("Astoria IDE needs to run from a folder you can write to — install per-user or move it out of Program Files") and exit, replacing today's silent write failures. ~15 contained lines; compile + launch check + a manual run from a read-only copy. **T2b:** delete the dead `Languages.txt` write sites (`frmFind.frm:575`, `frmFindInFiles.frm:476` — translation-era debug dumps, meaningless post-C4), per the cross-reference rule. **No settings-path changes; `SettingsService`/startup load order untouched** |
| 12 | 3 — robustness | **T16 — Adversarial review of the combined Wave-3 diff** | Fable | — | Review T2+T3+T4+T7 together before commit (they jointly touch every external-process launch and every settings write): injection, paths-with-spaces, failure paths, clipboard, threading. Findings go back to Sonnet; **commits held until this passes**, then one combined owner smoke test |
...
| 17 | 1 — hygiene (addendum, added 2026-07-11 per owner reminder) | **T17 — English-only shipped-content sweep** | Sonnet | F-C4 follow-through | Finish C4's English-only decision in shipped content: delete `Help/FB-manual-ru_RU-1.00.0.chm/.chw`, `Help/FB-manual-zh_CN-1.09.0.chm/.chw`, `Help/FB-manual-de_DE-1.00.0.chm/.chw`, `Help/Win32SDK.chinese.chm`, and `Help/Tip of the Day/chinese(Simplified).tip`, `chinese(Traditional).tip`, `french.tip` (all unreachable — `App.CurLanguage` hard-set `"English"`, `frmTipOfDay.frm:168-169` falls back to `english.tip`). Then remove the ru/zh/de entries from the shipped INI's `[Helps]` catalog — **⚠ `[Helps]` is one of the 7 indexed-key sections in the `SettingsService` loop (the 2026-07-03 infinite-loop class): renumber `Version_N`/`Path_N`/`Command_N` contiguously** (en_US stays 0; `win32_fb`, `VisualFBEditor`, `MyFbFramework` shift to 1/2/3) and keep `DefaultHelp` valid. `git grep` sweep for each deleted filename. **Out of scope (prior recorded decision):** `.lng`/language assets under `Controls/MyFbFramework/examples/**`, `Examples/**`, and `Tools/**` — third-party demo apps' own translations, not IDE capability. Sonnet because of the indexed-key renumbering; can run any time, natural slot alongside Wave 3 |
...
- **Sonnet (T2–T4, T7, T8, T12, T13):** one task = one contained diff, compile-clean per task. Wave-1 tasks (T8/T12/T13) commit individually per the session-end rule once verified. **Wave-3 tasks (T3/T4/T7/T2) are held uncommitted until T16's review passes** — then one combined owner smoke test covering: Open Containing Folder, an external tool from Tools ▸ External Tools, Delete Project (on a disposable project), Options ▸ Apply, and a clipboard-preservation check (copy text → do the above → paste).
