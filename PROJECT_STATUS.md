...
>
> Known defects (detail in the sub-project): **DR-1** `set_bp` pipe race (former T6 — attempt #2 code compiles clean, owner-verify *incomplete*, interrupted by the others); **DR-2** Step-Into no-highlight/no-scroll; **DR-3** rapid-F8 hang; **DR-4** ✅ FIXED + owner-verified 2026-07-11 (`b7af611`) — gutter-click blanking a `.frm` CodeAndForm view was a partial-repaint bug, unrelated to GDB/debug-mode. DR-2/3 are **not** caused by the `set_bp` change (it only touches the F9 path — see DR-10 below for why F9 doesn't even reach `set_bp` in normal use).
>
...
>
> **⏭️ CROSS-MACHINE HANDOFF (2026-07-11): Phase 0 (fragility audit) + Phase 1 (trace) are DONE and COMMITTED, and the FIRST OWNER REPRO PASS (all 5 obs) is DONE and analyzed — see [First repro-pass findings](#first-repro-pass-findings-2026-07-11-owner-obs-13-of-5). Net: DR-3 is really a DR-7 deadlock (do 2D before 2A); DR-4 is a decoupled editor-paint bug (1-line candidate fix); DR-1 reshaped into DR-10 (F9/gutter bypass `set_bp`); DR-2 didn't repro; DR-6 not yet exercised. New defects DR-8/9/10. Next action: a targeted SECOND repro pass for the still-untested paths (DR-1 via toolbar/menu "Toggle Breakpoint"; DR-6 via Stop while the program runs freely), then Phase 2 beginning with slice 2D. The trace log `Settings\debug_trace.log` is local-only (not git-synced).** Phase 0 added DR-6 (Stop/Kill races the worker + misuses `tlockGDB`) and DR-7 (worker mutates UI without marshaling) — see [`Documents/debugger_fragility_audit.md`](../../Documents/debugger_fragility_audit.md) (external, untracked — **not synced by git; re-generate or copy manually if needed on the other machine**). Phase 2 is reshaped around a single-owner **`GdbSession` transport** (slices 2A–2E). **The committed `astoria.exe` is the INSTRUMENTED build** — `DbgTrace` writes `tick + tid + tag + payload` to `Settings\debug_trace.log` (gated by `bDbgTrace=True` in `Debug.bas`; declares in `Main.bi`; helper bodies in `Debug.bas`; call sites in `Debug.bas`/`Main.bas`/`EditControl.bas`; own mutex `tlockDbgTrace`, never perturbs `tlockGDB`). **This instrumentation is temporary and MUST be stripped before Phase 2 fixes land.** Owner repro steps: (1) single Step-Into [DR-2], (2) gutter-click while stopped [DR-4], (3) F9 mid-session [DR-1], (4) mash/hold F8 until hang [DR-3 — expected hang], (5) Stop/End mid-session while stopped and while running [DR-6]. Then read `Settings\debug_trace.log` — the `tid=` column shows the worker-vs-UI interleaving. **Note: source files have MIXED line endings (LF-only lines inside CRLF files) — use byte-precise edits, not exact multiline string matches, for Phase 2.**
...
| **DR-11** | Closing a project **freezes the IDE when a non-project ("standalone") file is open** (**new 2026-07-11, owner-reported**) | Owner force-killed a frozen app, **relaunched fresh (no debug session)**; workspace-restore reopened the project *and* `Brush.bas` (a framework `#include` the debugger had auto-opened — see DR-8). Closing the project closed the project tabs, left `Brush.bas` open, and **froze** — deterministically (2/2). | **NOT a debug-lifecycle freeze** (fresh launch resets `CurEC` etc.); the debugger's only role is DR-8 opening the standalone file. **Root cause not yet pinned** — needs narrowing. Candidates in [`CloseProject`](src/Main.bas:2655) teardown: `NodeInProject` parent-walk ([`:2645`](src/Main.bas:2645)) looping on a malformed/cyclic restored-tab node; `tvExplorer_SelChange` fired by `Nodes.Remove` ([`:2814`](src/Main.bas:2814)); post-close activation/paint of the surviving standalone tab. | Likely a **general CloseProject bug** (any non-project file open, not just `Brush.bas`). Narrowing test pending (close standalone tab first → still freeze?; other project + standalone file → freeze?). Fix TBD once located; a project close should safely detach/keep non-project tabs. |
