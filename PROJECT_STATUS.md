...
> ✅ **DEBUGGER RELIABILITY SUB-PROJECT — CLOSED (all DR-1..DR-16 fixed + owner-verified).** See [⭐ ACTIVE SUB-PROJECT — Debugger Reliability](#-active-sub-project--debugger-reliability-queued-2026-07-11) for the full charter and defect table. ✅ **MYFBFRAMEWORK (MFF) REVIEW — CLOSED (all tasks done).** See [MyFbFramework review](#myfbframework-review-fable-2026-07-12) below.
>
> **Status at a glance (2026-07-12).** Both sub-projects opened this week are now fully closed and pushed to `main`:
> - **Debugger Reliability (DR-1 through DR-16):** every defect fixed and owner-verified, including the last two — **DR-16(a)** (`600a8c7`+`91b054f`, pre-flight GDB/exe/MainFile checks moved to the UI thread before any of the 5 debug-start entry points spawn the worker) and **DR-16(b)** (`deinit`'s worker-thread UI touches now marshal via `bDeinitCleanupPending`/`FlushDebugOutputOnUI`, the DR-7 pattern). The two-machine "reversion" scare (DR-4 gutter-scroll, the 2-clicks-to-close symptom) turned out to be **100% a testing artifact, not a regression** — see the gotcha below.
> - **MyFbFramework review (Fable, 2026-07-12):** all 6 real tasks done — widget drop (`4a0798b`: `HTTPServer`/`Animate`/`ListItemsOld` removed), T-OPUS-1 (`2aee074`: `ThreadsEnter`/`ThreadsLeave` contract formalized as no-ops, not a lock), T-OPUS-2 (`bbf8dc2`: `UString` audited, `AppendBuffer` fixed), T-SON-3 (`9331526`: unchecked-`Open` sweep), T-SON-4 (`f4d0ac5`: `Registry.bas` fixed, including a real UTF-16 write-corruption bug found along the way), T-SON-2 (`9b19f1e`: `HTTPConnection` hardened). The 3 remaining review tasks are moot (their target, `HTTPServer`, no longer exists).
>
> **What's still open (not urgent, no active work in progress):** (1) the Phase-4 **instrumentation-strip pass** — remove `bDbgTrace`/the remaining `DbgTrace` calls in `Debug.bas` now that the debugger sub-project's behavioral work is done; deliberately not done yet, needs an explicit owner go-ahead first (harder to undo than to add — see [Open Items](#open-items)). (2) A pre-existing gap in the non-debug `RunProgram`/`RunPr` path (no missing-exe check) and two owner UX requests (consolidate the Run menu, audit toolbar tooltips) — logged under [Open Items](#open-items). (3) MFF hygiene leftovers (F-H2: `README_CN.md`/`changes_cn.txt` safe to delete, but **`MyFbFramework.wiki/` is NOT baggage** — the IDE reads it live for component help — and `examples/`/`help/` need a usage check first), deferred. (4) **MFF hot-path deep-pass — DONE 2026-07-12 (Fable), 4 findings, none IDE-critical** (the feared GDI-handle leak did *not* materialize — the hot paths are solid); report `Documents/mff_hotpath_review.md` (local-only), fixes logged under [Open Items](#open-items) → MyFbFramework review.
>
> **⏭️ CROSS-MACHINE HANDOFF (2026-07-12, session ended on budget — resume here).** `git pull` first thing — synced to `origin/main` @ `7ff604c`, nothing uncommitted but the usual not-ours noise. Of the 4 hot-path findings: **H-3 and H-2 are DONE** (both turned into documented-and-deferred, not code fixes — see their Open Items entries for why: both were "fix has no IDE benefit + real risk to the working paint path," H-3 additionally a Chesterton's-Fence revert-risk). **H-1 is IN PROGRESS, not yet edited** — Sonnet read `Canvas.Cls` (`Canvas.bas:253-306`) and has a concrete fix plan written out in full in the H-1 Open Items entry (exact restructure: stop creating the brush `B` before the `FUseDirect2D` check; create/use/delete it only inside the non-D2D branch; make the D2D `Return` release-safe). **Pick up by re-reading that entry, then implement — no need to re-derive the plan.** **H-4 (the double-`FillRect`, `Canvas.bas:287/296/301`) is untouched**, trivial, Haiku-tier. **Both H-1 and H-4 are real code edits** (unlike H-2/H-3's comment-only commits) — **need a `mff64.dll` rebuild (`FORCE_MFF=1` via `Compile.bat`)** to take effect, then commit the rebuilt DLL alongside source. H-1's D2D path can't be live-verified in this IDE (force-disabled) — compile-clean is the only available check; say so in the commit.
>
...
>
> **⏭️ CROSS-MACHINE HANDOFF (2026-07-12 → next computer). `git pull` first thing.** Nothing uncommitted but the usual not-ours noise (`Controls/MyFbFramework/mff/Temp.bas`, `Settings/astoria.ini`, `Examples/DeviceExplorer/DeviceExplorer.vfp` — leave alone). **Nothing is mid-flight** — nothing was left half-fixed, no owner-response is being waited on beyond the deferred items listed above, and both sub-projects closed clean. This is a good point to start something genuinely new rather than continue anything. **⚑ Gotchas (carry forward):** (1) **Rebuild + commit a RELEASE exe (`Compile.bat`) immediately after every source fix** that touches `src/` or MFF files reachable from `mff.bi` — a prior session lost time to a stale committed exe. (2) `UseDebugger=false` in `astoria.ini` is STALE (written only on clean exit) — the live Run ▸ Use Debugger toggle is what counts. (3) A program with **no bound breakpoint runs to exit** (DR-8 removed break-at-entry); breakpoints on **comment lines** don't bind. (4) Trace logs are git-ignored/local-only — each machine accumulates its own. (5) Source files have MIXED line endings (LF-only lines inside CRLF files) — use byte-precise single-line `Edit` anchors, never large multiline string matches. (6) **If Claude launches `astoria.exe` via a script/automation, click the window's title bar once before testing anything** — Windows blocks a background-launched process from getting true foreground focus, and an unfocused window can produce misleading "regression" symptoms that are pure testing artifacts, not real bugs (this cost real time on 2026-07-12 before being tracked down). (7) **MFF-specific:** any edit to a file reachable from `Controls/MyFbFramework/mff/mff.bi` needs `FORCE_MFF=1` (via `Compile.bat`) to rebuild `mff64.dll`, or the change is invisible; not every `.bas`/`.bi` in that folder IS reachable — `Registry.bi` for example is orphaned source, not compiled in at all (confirmed via the include graph) — check `mff.bi`'s `#include` list before assuming a change needs (or will get) a DLL rebuild. (8) Supporting review/analysis docs (`fable_review.md`, `mff_review.md`, `debugger_fragility_audit.md`, etc.) live at `P:\Astoria-Docs\` (a synced pCloud drive, same path on both machines) — never in the git repo, so `git pull` won't bring new ones; they just need to already exist at that path.
...
### MyFbFramework review (Fable, 2026-07-12) — ✅ FULLY CLOSED 2026-07-12

**All tasks done.** T0 (widget drop: `HTTPServer`/`Animate`/`ListItemsOld` removed), T-OPUS-1 (`ThreadsEnter`/`ThreadsLeave` contract resolved), T-OPUS-2 (`UString` accounting audited + `AppendBuffer` fixed), T-SON-2 (`HTTPConnection` hardened), T-SON-3 (unchecked-`Open` sweep), T-SON-4 (`Registry.bas` fixed) are all committed and pushed. T-OPUS-3/T-SON-1/T-HAIKU-1 are moot (their target, `HTTPServer`, no longer exists). Commits: `4a0798b` (widget drop), `2aee074` (T-OPUS-1), `bbf8dc2` (T-OPUS-2), `9331526` (T-SON-3), `f4d0ac5` (T-SON-4), `9b19f1e` (T-SON-2). Full task-by-task detail in the checklist below; the source report is `P:\Astoria-Docs\mff_review.md`.
...
- [x] ~~**T-SON-2 — `HTTPConnection` hardening (F-N7).**~~ — **✅ DONE 2026-07-12 (Sonnet).** All three items: (1) `UserAgent` is now a public property (`HTTP.bi`, wired into `ReadProperty`/`WriteProperty` matching `Host`/`Port`/`Timeout`'s existing style), default changed from a hardcoded fake `Chrome/115` string to `"MyFbFramework-HTTPConnection"` (honest, doesn't impersonate a browser); `InternetOpen` now uses `This.UserAgent`. (2) New `MaxResponseSize` property (0 = unlimited, matches the prior unbounded default exactly); checked before each chunk is appended to `Responce.Body` so it can never exceed the cap — the chunk that would overflow it is dropped whole (not sliced mid-chunk) and the read stops there, noted in `Responce.Reason`. (3) Added a comment at the retry loop documenting that the 3×`Sleep(1000)` blocks the *calling* thread for up to 3s, with no protection from `ThreadsEnter`/`ThreadsLeave` (cross-referencing T-OPUS-1). `mff64.dll` rebuilt (`FORCE_MFF=1`, `HTTP.bi` **is** in the build graph unlike `Registry.bi`), IDE relinked + smoke-tested (launches clean). **Verification caveat — NOT live-network-tested:** attempted a local-listener round-trip (to directly observe the sent User-Agent header and exercise the size cap against a real response) but `HTTPConnection`'s existing (unchanged, out-of-scope) connect logic hardcodes port 80→plain-HTTP or any-other-port→HTTPS:443 regardless of the configured `Port` value, and binding port 80 for a local test listener needs admin rights this session doesn't have. Verified by code review + compile-clean + IDE smoke-test only; the changes are small, mechanical substitutions (pass a variable instead of a literal; a length-check guard before an existing append), lower-risk than the Registry.bas/UString bugs that got live-verified. **New minor finding, out of scope, not fixed:** the `Port=80→HTTP / else→HTTPS:443` coupling in `CallMethod` means the `Port` property can't actually select an arbitrary port — only "80" vs "not-80" matters. **(Sonnet)**
...
- [x] ~~**T-HAIKU-1 — remove debug `Print` in `HTTPServer.bas:165` (F-N6).**~~ — **MOOT: `HTTPServer` dropped.**
  ...
  **Hot-path deep-pass (Fable, 2026-07-12) — 4 findings; report `Documents/mff_hotpath_review.md` (local-only, hand-copy to sync). Headline: the feared GDI-handle leak did NOT materialize — `hbrBkgnd` is a correctly-managed shared global, the `Control` destructor ordering is safe, and the transparent-blit routine balances its save/restore/delete. Only H-3 has any live IDE impact.**
- [~] **H-3 (M) — `Control` `WM_PAINT` uses `GetDC` not `BeginPaint`/`EndPaint` (`Control.bas:897`). DEFERRED with a Chesterton's-Fence finding; in-code comment added 2026-07-12 (Fable).** Investigating the fix revealed the `BeginPaint`/`EndPaint` form **was already present, commented out — someone switched *from* `BeginPaint` *to* `GetDC` and left the original as dead code.** History check: that switch was made **upstream, before this fork existed** (the `-S` search lands on the fork's initial MFF-import commit `bef9267`, not a paint change), so the rationale is **not recorded anywhere we can see.** The current `GetDC` code is **not actively broken** (the IDE paints fine, incl. the code tab strip) — the "issues" are a bounded full-client-repaint inefficiency + a latent trap (infinite `WM_PAINT` loop only if someone later sets `Message.Handled` here). **Decision: do NOT re-apply `BeginPaint` blind** — re-introducing a reverted upstream change into IDE-visible paint code without knowing why it was reverted (suspected flicker/clip interaction) is the wrong risk. Instead, the commented-out `BeginPaint` dead code was **removed and replaced with a comment** documenting both invariants (don't set `Handled`; the revert history + verify checklist if ever retried). Comment-only change, no behavior change, no rebuild (mff64.dll byte-identical). **If pursued later:** Opus session with iterative live-verify across all `OnPaint` surfaces (code tab strip, options color panel, search-box icons, menu editor), A/B'ing `BeginPaint` vs `GetDC` for flicker/clip — only if partial-repaint cost ever shows as a real symptom.
- [ ] **H-1 (M, D2D path only — IDE force-disables D2D) — `Canvas.Cls` leaks an `HBRUSH` on the Direct2D early-return (`Canvas.bas:253-306`). IN PROGRESS 2026-07-12 (Sonnet) — read the code, not yet edited; picks up here.** Current structure (verbatim, current line numbers): `Cls` computes `Handle_ = GetDevice` (only if `Not HandleSetted`, :255) then unconditionally does `Dim As HBRUSH B = CreateSolidBrush(FBackColor)` (:257) *before* branching on `FUseDirect2D`. The D2D branch (:258-264) clears via `pRenderTarget->Clear` and `Return`s (:264) — **skipping both `DeleteObject B` (:303) and `If Not HandleSetted Then ReleaseDevice Handle_` (:305)**, so a D2D `Cls` leaks the brush *and*, if `HandleSetted` was false, the acquired device too. **Fix shape:** move `CreateSolidBrush` so it's created only on the paths that use `B` (the two GDI-fill branches, :287/296, and the unconditional fallthrough fill at :301) — i.e. don't create `B` before the `FUseDirect2D` check; create it only in the `Else`/non-D2D path, right before the fills that consume it, and keep `DeleteObject B` scoped to match. Simplest: restructure so the D2D branch's `Return` at :264 becomes `ReleaseDevice`-safe (call `If Not HandleSetted Then ReleaseDevice Handle_` before that `Return`, since no brush was created yet if creation is moved past the D2D check) — then `B`'s creation/use/delete all live inside the non-D2D branch, symmetric and leak-free by construction. **Verify:** compile-clean is the only verification possible in this IDE — the D2D path is force-disabled (`SettingsService.bas:174-179`), so this can't be live-exercised here; note that explicitly in the commit. Needs a `mff64.dll` rebuild (`FORCE_MFF=1` via `Compile.bat`) to take effect — this is a real code change, unlike the H-2/H-3 comment-only commits. **(Sonnet)**
- [~] **H-2 (M, latent — standalone-Canvas only, IDE unaffected) — `Canvas` `GetDevice`/`ReleaseDevice` state machine inverted (`Canvas.bas:339,398`). DOCUMENTED + DEFERRED 2026-07-12 (in-code note added).** Root cause: `HandleSetted` conflates two colliding meanings — (1) "DC set externally, control owns it, don't release" (the IDE's usage) and (2) "we hold a DC we acquired via `GetDevice`." `GetDevice` latches it True for (2), but `ReleaseDevice` reads it for (1) and bails → a standalone Canvas leaks one DC per lifetime (not per-call; not even released in the destructor). **Deliberately NOT fixed:** the IDE never hits this path (its controls always set `HandleSetted` externally + own the DC — confirmed in `EditControl.bas`/`Designer.bas`), so a fix has **zero IDE benefit** while risking the **83 `HandleSetted` checks** the working control-owned paint path depends on; and the only affected pattern (standalone Canvas) is exercised **nowhere in the IDE/examples**, so a fix **cannot be live-verified here.** Correct fix (if ever pursued on Opus): add a **second flag** (`FOwnsDevice`, set only when `GetDevice` acquires) so release only touches Canvas-owned DCs — with a standalone-Canvas test harness, not blind. Full rationale in the in-code comment at `GetDevice` and in `Documents/mff_hotpath_review.md`.
- [ ] **H-4 (L) — `Canvas.Cls` fills the rect twice on the GDI path (`Canvas.bas:287/296` + `301`).** Redundant paint every `Cls`; drop the duplicate. **(Haiku)**
- [ ] **T-DEFER (MFF) — not blocking:** widget-trimming **partially done** (F-H3 — `HTTPServer`/`Animate`/`ListItemsOld` dropped 2026-07-12; the remaining ~12–17 keep-listed widgets are deliberately retained per the T0 note). **CN/example baggage (F-H2) — ⚠️ partially a landmine:** `README_CN.md`/`changes_cn.txt` are safe to delete, but **`MyFbFramework.wiki/` is NOT baggage — the IDE reads it live for component help (`Main.bas:5420` `wikiFolder`), so deleting it removes toolbox component docs;** `examples/`/`help/` need a usage check first (not yet done). `Canvas` GDI-handle audit (F-R-gdi) — **done 2026-07-12 (hot-path deep-pass above): no long-session leak found in the IDE's paths; the 4 findings are D2D/standalone-Canvas/custom-paint, none the feared slow IDE leak.** Revisit the rest with owner appetite.
