...
**Last updated:** 2026-07-07 (Opus "Next Steps" backlog Phase A + B + C + D(partial) + E done & committed: robustness fixes, AI thread responsiveness, off-thread `Suggestions`/narrowed `FormatProject` disable, calmer debugger messages, owner-decided fresh-install starter project, Options-dialog jargon cleanup, feedback-channel policy + two silent-failure fixes, owner-decided AI catalog trim + Android/ADB removal; 13.3.A S1–S7 **all Opus-reviewed & committed**; owner-found Run-toolbar persistence regression fixed; Edit menu owner-approved; Search → Define F2 reliability pass; File/Open Project + path fixes)  
...
**Active work:** Opus's "Next Steps" review (`Next Steps - Opus.md`, 2026-07-07) evaluated three prior AI reviews against the actual source and produced a verified, sequenced backlog. **Phase A + P1 + Phase B (P2/P3) + Phase C (U1/U2/partial U3-U4) + Phase D (F1, F2) + Phase E (O1, O3) are done and committed** (see below); **R5 (bounded GDB read)** and **E1 (Apply dirty-tracking, spun off as its own task)** deliberately deferred. 13.3.A S1–S7 are **all committed** (Opus-reviewed). A Run-toolbar persistence regression the owner found post-S4 (my own S3 migration latching Run permanently visible) was root-caused and fixed 2026-07-07 — see the S5–S7 review outcome below. Next: the remaining Phase C static-event-handler/`.lng`-maintenance decisions, R5 (needs a dedicated live-GDB session), E1 (dirty-tracking task), or resume §13.3 step-by-step UI review with the **View menu** (File + Edit complete; Search → Define reliability pass shipped with S1–S4).  
**Open items consolidated:** see [Open Items](#open-items) below.

---

## Opus "Next Steps" backlog — Phase A + P1 + Phase B + Phase C + Phase D(partial) + Phase E(partial) done (2026-07-07)

Executes Phase A, item 5 (P1), Phase B (P2/P3), Phase C (U1/U2, partial U3-U4), Phase D (F1, F2), and Phase E (O1, O3) of `Next Steps - Opus.md`'s consolidated, source-verified backlog (Opus cross-checked three prior AI reviews — Cursor, Deepseek, Sonnet — against the actual code before sequencing this plan; see that document for full methodology and the discarded/unsubstantiated claims).
...
- **E1** — `cmdApply_Click`'s ~400 unconditional INI writes (96 distinct `WriteXxx` call sites, many in loops over AI agents/terminals/editors/helps/build configs/include+library paths): **owner wants this fixed**, but it's a large, delicate rewrite of a ~400-line Sub, not a quick win — spun off as its own background task (`task_ea2834b6`, "Add dirty-tracking to Options Apply INI writes") rather than attempted inline this session.

**Remaining Opus backlog** (not started, see `Next Steps - Opus.md` for full detail): rest of Phase C (static-event-handler default, `.lng` maintenance decision), R5 (needs a dedicated live-GDB session — see above), E1 (spun off as a background task, see above), Phase F (deferred structural: file splits, `FormDesign` debounce — deliberately last, touches the fragile designer path).
