...
>
> **⏭️ SESSION HANDOFF (2026-07-12, ~8h break — resume here).** **Done + owner-verified this session:** slice **2A** (DR-3 desync), **DR-4** (regressed-then... see below), **DR-9**, **DR-12**, and **slice 2B (DR-6 + DR-14)** — Stop-while-running now kills the inferior race-free (`0f30654`; trace-confirmed: pid captured from `[New Thread]`, no `kill_debug.terminate`, `LOOP.inferiorGone`, worker-side `deinit`). Slices **2A+2D** (from the other machine) fixed DR-3's other mode + DR-13. **The committed `astoria.exe` is still fully instrumented** — GDB `DbgTrace` (`bDbgTrace=True`, `Debug.bas:362`) + the DR-11 `CloseProject`/`SelChange` markers in `Main.bas` (DR-11 is CLOSED, so those markers are strippable now). **New reusable debug target: `Projects/Project3`** = a console loop (`*File=Module1.bas` main file set) that runs freely and stops at breakpoints — use it for the remaining repros (much better than the FileBrowser GUI app, which spuriously SIGSEGVs under GDB on `RtlSleepConditionVariableCS`). **⚑ Key gotcha learned this session:** GDB's `interrupt` does NOT stop a freely-running inferior in all-stop sync mode (GDB isn't reading stdin while the inferior runs) — you MUST `TerminateProcess` the inferior; and `iGlPid` is only captured after a stop *unless* you grab it from the `[New Thread PID.TID]` startup line (now done in `readpipe`). **Next up, in order:** **(1) slice 2C — DR-1/DR-10:** route F9/gutter/menu breakpoint-toggle into one arm-during-debug path (enqueue, not direct pipe write); verify hit-bp / comment-line refusal / toggle-off. **(2)** confirm **DR-2** (step no-highlight) stays resolved. **(3) DR-4 regression** (split-view gutter blank came back; `b7af611` fix still present — suspect debug-session/theme-specific; Sonnet-appropriate). **(4)** DR-7 residuals (worker-side `ShowMessages`/`UpdateWatch`). **(5) Phase 4:** strip ALL trace instrumentation (GDB `DbgTrace` + CloseProject markers) + dead-code sweep (incl. the now-unused `kill_debug`, and `timer_data`). Also a **breakpoint-clear bug** surfaced (Ctrl+Shift+F9 didn't clear a line-2 breakpoint; use Run ▸ More Debug Options ▸ Clear All Breakpoints menu — likely DR-10 family). **Trace logs are git-ignored / local-only.** **Source files have MIXED line endings — byte-precise edits only.**
...
| **DR-4** | ⚠️ **REGRESSED — owner reports the split-view gutter blank is BACK (2026-07-11), deferred by owner.** Was fixed in `b7af611`, owner-verified once | Clicking the gutter to toggle a breakpoint makes lines disappear in a `.frm` **CodeAndForm (split) view** until refocus | The `b7af611` fix (`PaintControl True` on toggle) **is still present** in `EditControl.Breakpoint` ([`EditControl.bas:242`](src/EditControl.bas:242)) — so it wasn't reverted, and no later change touches the editor paint path. So the earlier verify covered a scenario the fix handles, but another does not: **suspect (a) it only blanks during an active debug session, or (b) it's theme-dependent (owner's hypothesis — needs checking across themes).** Already instrumented (the `EC.Breakpoint` trace logs `CurExecutedLine`/`CurEC`). | **Owner deferred — do not fix yet.** When resumed: reproduce in the current theme + a couple of others, capture the `EC.Breakpoint` trace (is `PaintControl True` reached? what's the paint state?), and check whether `PaintControl True` is genuinely a *full* repaint in the divided layout or whether `PaintControlPriv` still skips a pane. |
