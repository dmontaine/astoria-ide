...
> ⏸️ **DEBUGGER RELIABILITY SUB-PROJECT — ACTIVE (WIP, synced across machines via git).** See [⭐ ACTIVE SUB-PROJECT — Debugger Reliability](#-active-sub-project--debugger-reliability-queued-2026-07-11) for the full charter and defect table (DR-1..DR-13).
>
> **Status at a glance (2026-07-12):** ✅ **FIXED + owner-verified:** **DR-1 + DR-10 (mid-session breakpoint arm/clear) via slice 2C** (owner-verified 2026-07-12) — F9/gutter/menu now route through one enqueue-based `arm_breakpoint` path; the worker (single pipe owner) applies break/clear in lockstep, so mid-session arm-on stops at the newly-armed line and toggle-off actually removes it. **Six sub-fixes shook out during verify** (all in): (a) the `armbp` branch must release `tlockGDB` before looping or the next `EnqueueDebugCommand` deadlocks the UI; (b) **DR-14 residual** — a program that exits with `Program exited...`/`* 1 <null>` (this GDB's console-app exit) slipped past the 2B `[Inferior]`/`not being run` check → now also bails on no-live-process in `info inferiors`; (c) **breakpoint-clear bug (was the deferred DR-10 residual)** — `clear LINESPEC` NEVER matches on this toolchain (verified with gdb 11.2: full path, basename, and the exact uppercased `MODULE1.BAS:20` all return "No breakpoint at"); fixed by recording each breakpoint's GDB number at set-time and `delete <N>` on clear; (d) `delete N` is SILENT in `-f` fullname mode (bare `(gdb) ` prompt) → must read with `WithoutAnswer=True` or `readpipe` spins forever; (e) **the core 2C bug** — `EditControl.Breakpoint` read `FECLine->Breakpoint` *after* `PaintControl` reused `FECLine` as its paint-loop var, so every mid-session toggle enqueued `clear` (arm-on never worked); capture the state before painting. **DR-3 fully (both hang modes)** — desync via slice **2A** (`1899513`) + the DR-7 **deadlock** via slice **2D** (2026-07-11, worker→UI panel marshal + removed off-thread `DoEvents`; owner "works, no problems"); **DR-13** (swallowed step) via 2D part-1; **DR-8** (break-at-entry removed → runs to user breakpoints; + paired UX: close-on-stop for debugger-opened files, and Close Project = clean slate; owner-verified 2026-07-11), DR-9 (stale committed example binaries, `6223698`), DR-12 (dead toolbar Toggle-Breakpoint button, `d8a2e8f`); **DR-6 (Stop/Kill race) + DR-14 (dead-inferior refresh hang) via slice 2B** (`0f30654`, owner-verified 2026-07-12 — Stop-while-running-freely now terminates cleanly, no `kill_debug` UI-thread race); **DR-11** (CloseProject freeze) — **RESOLVED as a downstream symptom of DR-3** (no longer reproducible post-fix; 3 clean `CloseProject` runs, no code change needed). **🔑 Key discovery (2026-07-11): `ThreadsEnter`/`ThreadsLeave` are no-op stubs (`Component.bas:257`)** — so *every* worker "marshaled" UI update was always an unsynchronized race; 2D moves the debug-panel fills to the UI thread (`TimerProcGDB`→`FillDebugPanelsOnUI`) and the worker only does pipe I/O. ⏳ **Still open:** **DR-4 (disappearing text on breakpoint toggle) → the next item, now with a CLEAN repro (2026-07-12):** click a line, F9, and ~half the viewport text blanks until any refocus (click in the text area) restores it. **Reproduces on a plain single-pane `.bas`** (Project3/Module1.bas) — so the old "`.frm` CodeAndForm split-view only" / theme-specific hypothesis is wrong; `PaintControl True` in `EditControl.Breakpoint` simply isn't repainting the whole viewport. Cosmetic/non-fatal (refocus fixes it); not caused by the 2C change. DR-2 (step no-highlight) → did NOT reproduce in 2C verify; highlight fired correctly on every stop; treat as resolved-unless-it-recurs. DR-7 residuals (worker-side `ShowMessages`/`UpdateWatch`) still open. Phase 4 (strip instrumentation + dead-code sweep incl. the now-fully-dead `set_bp`) still pending.
>
> **Standing rule (non-negotiable, carried over from the original T6 charter): instrument-first — add file-trace, have the owner reproduce, read the actual interleaving, *then* fix. Do NOT reorder/restructure the GDB worker loop without a confirming trace** (attempt #1 hard-locked doing exactly that; 2A's trace-confirmed minimal gate is the model to repeat for 2B–2E).
>
> **⏭️ SESSION HANDOFF (2026-07-12 — resume here).** **Done + owner-verified this session: slice 2C (DR-1 + DR-10)** — F9/gutter/menu breakpoint toggles now all route through one enqueue-based `arm_breakpoint` (`Debug.bas`); the debug worker (single pipe owner) applies break/clear/tbreak in its lockstep loop (new `LOOP.armbp` branch), so mid-session **arm-on stops at the newly-armed line** and **toggle-off actually removes the breakpoint**. Verified live on `Projects/Project3` (now a longer console test bench — factorials/Fibonacci/heartbeat, with comment+blank lines for the refusal test). **Six fixes landed (all owner-verified together):** (1) 2C itself; (2) `armbp` releases `tlockGDB` before looping (else the next Continue/Step `EnqueueDebugCommand` deadlocks the UI — first freeze found); (3) **DR-14 residual** — bail on `info inferiors` showing no live process (`* 1 <null>`) + on `Program exited` string, since this GDB's console-app exit doesn't say `[Inferior]`/`not being run` (that was a second freeze); (4) **DR-10 clear bug FIXED** — `clear LINESPEC` never matches on this toolchain (proved directly with the bundled gdb 11.2: full path, basename, and exact `MODULE1.BAS:20` all "No breakpoint at"); now record each bp's GDB number at set-time (`BpMap*` in `Debug.bas`) and `delete <N>` on clear; (5) `delete N` is SILENT in `-f` fullname mode (bare `(gdb) `) → read with `readpipe(True, …)` or it spins forever (third freeze); (6) **the core 2C bug** — `EditControl.Breakpoint` read `FECLine->Breakpoint` *after* `PaintControl` clobbered the shared member `FECLine` (it's the paint-loop var, `EditControl.bas:4274`), so every toggle enqueued `clear` and arm-on never worked → now captured into a local before `PaintControl`. **`set_bp` is now fully dead** (superseded by `arm_breakpoint`) — delete in Phase 4. **The committed `astoria.exe` is a release build, still fully instrumented** (`bDbgTrace=True`, `Debug.bas`). **⚑ Gotchas for next time:** `UseDebugger=false` in `astoria.ini` is STALE (written only on clean exit) — the live Run ▸ Use Debugger toggle is what counts; a program with **no bound breakpoint runs to exit** (DR-8 removed break-at-entry), so always set a breakpoint on a real *code* line before F5; breakpoints on **comment lines** don't bind. **Next up, in order:** **(1) DR-4 — the disappearing-text bug (clean repro now):** F9 on a line blanks ~half the single-pane `.bas` viewport until refocus; `PaintControl True` in `EditControl.Breakpoint` isn't repainting the whole viewport (the old `.frm`-split/theme hypothesis is disproved). Instrument-first: capture the `EC.Breakpoint`/paint state and check `PaintControlPriv`'s skip logic. **(2)** DR-2 looks resolved (highlight fired on every 2C stop) — close unless it recurs. **(3)** DR-7 residuals (worker-side `ShowMessages`/`UpdateWatch`). **(4) Phase 4:** strip ALL trace instrumentation (`DbgTrace` + any leftover `CloseProject`/`SelChange` markers) + dead-code sweep (`set_bp`, `kill_debug`, `timer_data`). **⚠️ NEW OBSERVATION (2026-07-12, unverified — worth a trace):** an `astoria.exe` process **survived several minutes after the window was closed** (owner confirmed the IDE was already closed, yet PID 19912 still held `astoria.exe` locked, failing a rebuild). A clean shutdown should terminate the process — smells like a **thread not exiting on close** (debug worker / timer thread keeping the process alive), consistent with the `FormClosing` bail paths. Candidate new DR item ("orphaned process on close"); reproduce by closing the app and checking Task Manager for a lingering `astoria.exe`, then trace the shutdown. **Trace logs are git-ignored / local-only. Source files have MIXED line endings — byte-precise edits only.**

**Last updated:** 2026-07-11 — **Fable review remediation CLOSED and archived.** All tasks T1–T17 done and the owner smoke-test list completed; the full task table now lives in [HISTORY.md](HISTORY.md) (T14 archival), with a summary stub of the load-bearing outcomes below. Highlights: T1 owner decision **Option B — per-user install** (portable model kept, **§13.5 unblocked**); Wave 3 robustness fixes committed individually; T16's 3 findings fixed; T5 GDB-path quoting owner-verified against a spaced path; T15 themes curated 96 → 10 (`9c18bd8`); the owner-found shared-`frmPath` `ModalResult` bug (silently breaking every "Add" dialog) fixed (`7a0c829`); Step Out promoted to the top-level Run menu with clarifying step tooltips. **Debugger work is now its own active sub-project** — see [⭐ ACTIVE SUB-PROJECT — Debugger Reliability](#-active-sub-project--debugger-reliability-queued-2026-07-11) (DR-1..DR-5). Full dated session-by-session narrative is archived in [HISTORY.md](HISTORY.md).
...
| **DR-4** | 🔜 **THE NEXT ITEM — clean repro as of 2026-07-12.** (Was: "split-view gutter blank, regressed, deferred.") | Toggle a breakpoint (F9 or gutter) and **~half the viewport text disappears** until any refocus (click in the text area) restores it. | **Old hypothesis DISPROVED (2026-07-12):** it is **NOT** `.frm` CodeAndForm-split-only and **NOT** theme-specific — it reproduces on a **plain single-pane `.bas`** (`Projects/Project3/Module1.bas`). So `PaintControl True` in `EditControl.Breakpoint` ([`EditControl.bas`](src/EditControl.bas:241)) simply is **not repainting the whole viewport** even in the simple single-pane case. Cosmetic/non-fatal (refocus restores). Not caused by the 2C `FECLine` capture change (that's a pure read added before the same `PaintControl True`). | **Instrument-first when resumed:** capture the `EC.Breakpoint` paint state, then look at `PaintControlPriv` (~[4295](src/EditControl.bas:4295)): does `PaintControl True` actually force a full-viewport repaint, or does the partial-paint skip still leave the lower/upper half cleared-but-not-redrawn? A refocus triggers a full paint (that's why clicking restores it), so the toggle path's paint is incomplete. |
...
- **Phase 3 — full re-verify.** Owner runs the complete checklist on the final build (set bp → step → inspect → continue → stop; F9 mid-session; gutter-click; rapid step; Stop at each state) before behavioural work closes.
- **Phase 4 — dead-code sweep (separate, low-risk, *after* the fixes land).** `timer_data` (~165 dead lines), `hard_closing`, the integrated/stabs state block (incl. the 5 MB `sourcebuf`), commented cruft, dead `iTime` params — each deletion §9 cross-referenced (**keep** `prun`/`dbghand`/`kill_process`, the live Run-without-debug path), one compile-clean commit. Refresh `THREADING.md`'s stale debug-worker entry (`start_pgm`/`attach_debuggee` don't exist; real path is `RunProgramWithDebug`→`run_debug`) here too.
...
- [ ] **Theme re-curation (owner picking their own set)** — *2026-07-11, cross-machine handoff.* Owner didn't want T15's auto-picked 10 themes. **All 96 original themes are temporarily committed to `Settings/Themes/`** (restored from `9c18bd8^`) so they can be browsed live in **Options ▸ Themes** (the app scans the folder at runtime — no rebuild needed). **Next session (owner's other machine): owner browses, names the themes to keep, then re-curate** — `git rm` the rest, and make sure the shipped `CurrentTheme` (currently `Default Theme`) points at a kept theme. The current 96-theme state is intentional-but-temporary; do not treat it as the final shipped set.
