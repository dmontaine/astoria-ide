...
>
> **Status at a glance (2026-07-11):** ✅ **FIXED + owner-verified:** DR-3's pipe-desync hang mode (slice **2A**, `1899513`), DR-4 (`.frm` split-view gutter blank, `b7af611`), DR-9 (stale committed example binaries, `6223698`), DR-12 (dead toolbar Toggle-Breakpoint button, `d8a2e8f`). ⏳ **Still open:** DR-3's *other* hang mode (a DR-7 deadlock) + DR-13 (swallowed step keypress) → both fixed by slice **2D** (marshal worker→UI, delete off-thread `DoEvents`) — not started. DR-1 (mid-session breakpoint arm, reached via Run ▸ Toggle Breakpoint menu only — see DR-10) → slice **2C**, checklist incomplete (hit-bp / comment-line-refusal / toggle-off not yet observed). DR-6 (Stop/Kill race) → slice **2B**, not yet exercised live. DR-2 (step no-highlight) → hasn't reproduced since DR-9's stale-exe was fixed; may already be resolved, unconfirmed. DR-8 (session always breaks at `#include`d entry) → small UX fix, not started. **DR-11 (CloseProject freezes the IDE when a non-project file is open) → instrumented and committed (`f534b42`), NOT YET REPRODUCED — see cross-machine handoff below — root cause not yet pinned.**
>
...
>
> **⏭️ CROSS-MACHINE HANDOFF (2026-07-11, mid-session — switching machines):** **The committed `astoria.exe` carries BOTH sets of instrumentation**: (1) the original GDB `DbgTrace` (gated `bDbgTrace=True` in `Debug.bas:362`, writes to `Settings\debug_trace.log`) — still live, used for all debugger repros; (2) **NEW** `DbgTrace` markers through `CloseProject`'s teardown + `tvExplorer_SelChange` in `src/Main.bas` (9 markers: `CloseProj.enter/tabsClosed/safetyPassed/nodesFreed/beforeNodesRemove/afterNodesRemove/clearAnalysis/clearDebug/changeUseDbg/changeMenus/done`, `SelChange.enter/beforeOpen/afterOpen`) — added for **DR-11**, **committed as temporary instrumentation** (same pattern as the original trace build) so it survives this machine switch; strip both sets once their diagnoses are done. **Next action on the other machine: reproduce DR-11** — File ▸ Open Project → `Examples/FileBrowser/FileBrowser.vfp`, make sure a non-project file is open (e.g. `Brush.bas`), close the project, force-kill if it freezes, then read `Settings\debug_trace.log`'s last `CloseProj.*`/`SelChange.*` line before the freeze. **After DR-11 (or in parallel if machine time allows): slice 2D** is the next fragile-core fix (marshal worker→UI + delete the off-thread `Updateinfoxserver`/`DoEvents` in `Debug.bas` — fixes DR-3's deadlock mode and DR-13). Also still pending: a completed DR-1 checklist and a live DR-6 (Stop-while-running) test, both blocked only by session time, not by any known defect now that 2A stopped the wedging. **Trace logs (`Settings\debug_trace.log`, `*.pass1.log`, `*.pass2.log`) are local-only, NOT git-synced** — each machine accumulates its own; archive/rename before a fresh repro to keep captures separable. **Note: source files have MIXED line endings (LF-only lines inside CRLF files) — use byte-precise edits, not exact multiline string matches, for any further Phase 2 work.**
...
- **T15 owner decision (Option B — curated shortlist):** shipped editor themes cut 96 → 10 (`9c18bd8`); shipped default reset to `CurrentTheme=Default Theme`. Memo: [`Documents/fable_t15_theme_catalog.md`](../../Documents/fable_t15_theme_catalog.md).
...
| **DR-7** | Worker thread mutates UI without marshaling (**new 2026-07-11, fragility audit**) | Latent corruption/crash; also a live suspect for DR-2/DR-4's missed repaints | root-caused **statically** (rule-3 violations): (1) `readpipe`→`ShowMessages` (`:486`) writes `txtOutput`/`frmMain` from the worker on every stop-read (`ShowMessages` self-marshals nothing, `Main.bas:8716`); (2) `Updateinfoxserver(100)` (`:2140`) runs `pApp->DoEvents` ×101 **off-thread** — pumps the message loop from the worker (prime suspect for missed paints between `fcurlig` set and `TimerProcGDB`); (3) `RefreshDebugPanelsAfterStop` holds `ThreadsEnter` across blocking `readpipe` loops (`:1992`) → UI stall | **Do alongside DR-2/DR-4** (off-thread `DoEvents` is a paint-pipeline suspect): route worker→UI updates through the marshaling path, drop the off-thread `DoEvents`. See audit §A. |
| **DR-8** | Debug start always breaks at program entry, landing inside an `#include`d file (**new 2026-07-11, trace pass**) | Starting a debug session (with or without user breakpoints) stops at the first executable statement, which — for a form app — is a module-level global initializer pulled in via `#include` (e.g. `Brush.bas:16 g_brItemBackground = CreateSolidBrush(...)`), *not* the user's own code. Confusing: "why am I in Brush.bas?" | root-caused **statically** (confirmed by trace): [`run_debug`](src/Debug.bas:2054) unconditionally sends `b 1\n` every launch → GDB breaks at `main` entry; first source-mapped line after the prologue is the include's module-init. Independent of user breakpoints. | `Debug.bas:2054` (`b 1`). UX design call: run-to-first-breakpoint vs. always-break-at-entry; and/or skip framework `#include` init when stepping. Not a race/transport bug. |
...
| **DR-11** | Closing a project **freezes the IDE when a non-project ("standalone") file is open** (**new 2026-07-11, owner-reported**) | Owner force-killed a frozen app, **relaunched fresh (no debug session)**; workspace-restore reopened the project *and* `Brush.bas` (a framework `#include` the debugger had auto-opened — see DR-8). Closing the project closed the project tabs, left `Brush.bas` open, and **froze** — deterministically (2/2). | **NOT a debug-lifecycle freeze** (fresh launch resets `CurEC` etc.); the debugger's only role is DR-8 opening the standalone file. **Root cause not yet pinned; ⏳ INSTRUMENTED 2026-07-11 (Sonnet), owner repro PENDING (cross-machine handoff — see banner).** 9 `DbgTrace` markers added through `CloseProject` teardown + `tvExplorer_SelChange` in `src/Main.bas` (committed as temporary instrumentation, same convention as the GDB trace build). Candidates the markers will discriminate between: `NodeInProject` parent-walk ([`:2645`](src/Main.bas:2645)) looping on a malformed/cyclic restored-tab node; `tvExplorer_SelChange` fired by `Nodes.Remove`; post-close activation/paint of the surviving standalone tab. | Likely a **general CloseProject bug** (any non-project file open, not just `Brush.bas`). Repro: open the FileBrowser project with a non-project file open, close the project, force-kill if it freezes, then read `Settings\debug_trace.log`'s last `CloseProj.*`/`SelChange.*` line before the freeze. Fix TBD once located; a project close should safely detach/keep non-project tabs. |
...
- **Phase 2 — build the `GdbSession` transport in verifiable slices.** The transport rewrite delivered as a sequence, each slice compile-clean + owner live-verify + one commit; the command surface is unchanged so every slice runs against the current baseline (no big-bang valley). Dependency-driven order:
  ...
  - **2C — mid-session commands onto the queue (DR-1).** `set_bp`/`break_debug` (and any other mid-session direct-pipe caller) enqueue instead of touching the pipe; the worker applies breakpoints at a safe point. This is attempt-#1's territory but now *on top of* a lockstep loop that owns the read side — exactly what attempt #1 lacked. *Verify:* DR-1 F9 checklist (set bp mid-session → step → run; comment-line refusal; toggle-off).
  - **2D — marshal worker→UI (DR-7).** Route output/panel/highlight updates through the marshaling path; delete the off-thread `Updateinfoxserver`/`DoEvents`; drop `ThreadsEnter`-across-`readpipe` in the watch refresh. *Verify:* Output panel populates under stepping; **check whether this alone resolves DR-2/DR-4.**
