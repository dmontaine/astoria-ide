...
**Last updated:** 2026-07-11 (**T9–T11 complete, Wave 1 hygiene done** — Haiku executed T9 (repo hygiene sweep: deleted Downloads path from `src/Temp.rc`, untracked + gitignored three scratch files, added five ignore rules), T10 (cleaned tracked INI: stripped RecentSession/MRU lists/geometry keys), and T11 (deleted four unreachable Chinese data files). Compile-clean verification passed. Ready for T8/T12/T13 (Wave 1 finalization) and then T1 decision gate (settings-location model).)
...
**Also active:** the **Fable review remediation queue (T1–T16, 2026-07-11)** — see "⭐ ACTIVE SUB-PROJECT — Fable review remediation" below for the execution order and per-model instructions. **Wave 1 (hygiene, T9–T11, T8/T12/T13) done/in-progress** — compile-clean verified. Next: Sonnet Wave 1 finalization (T8 CI cleanup, T12 README, T13 manifest + version 1.0), then **T1 decision gate** (settings-location model) before T2 and §13.5 proceed. **§13.5 (installer) is blocked on T1/T2** (settings out of `ExePath`).
...
- **§13.5 (installer) is blocked on T1 + T2**: settings, workspace, temp, and logs are all written under `ExePath` (`Main.bi:30` `SettingsPath`); under Program Files those writes fail for a standard user, and because the app is 64-bit, UAC file virtualization does **not** apply.
...
| 7 | 2 — decision | **T1 — User-data location decision memo** | Fable | F-S4, F-C1 | Design memo: `%APPDATA%\AstoriaIDE\` migration (first-run copy-from-ExePath, honoring the §9 INI-migration rule) vs. a declared portable-install model. Must cover every ExePath-relative write: `SettingsPath` (`Main.bi:30`), `Settings/Workspace.ini`, `Temp/` (`Main.bas:8326`, `BuildService.bas:195`), `Logs/`, `Tools/Tools.ini` (`frmTools.frm:241`), `HotKeys.txt` (`frmOptions.frm:3119`). Save to `Documents/`; **owner sign-off gates T2 and §13.5** |
...
| 11 | 3 — robustness | **T2 — Implement the settings-location migration** | Sonnet | F-S4 | Execute exactly the model T1's owner-signed memo specifies — no improvised path layout; every relocated file gets a migration read of its old location per §9's INI-migration rule |
