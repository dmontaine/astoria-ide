...
- [ ] **H-2 (M, latent — standalone-Canvas only, IDE unaffected) — `Canvas` `GetDevice`/`ReleaseDevice` state machine is inverted (`Canvas.bas:339,379`).** `GetDevice` latches `HandleSetted=True`; `ReleaseDevice` guards `If HandleSetted Then Exit Sub` and never resets it → a standalone Canvas grabs one DC and never releases it (not even in the destructor). IDE unaffected (its controls set `HandleSetted=True` + own the DC). Fix needs a real two-state model without regressing the control-owned path. **(Opus)**
