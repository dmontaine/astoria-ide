...
**Remaining Opus backlog:** R5 **done** (2026-07-08, see above); E1 (running as background task `task_ea2834b6`); Phase F (deferred structural: file splits, `FormDesign` debounce — deliberately last, touches the fragile designer path; not started, low priority, see `Next Steps - Opus.md`). Everything else in the Opus backlog is done.
...
**D1 form-close greying gap — now fixed** (see the D1 outcome section above; resolved via `ChangeMenuItemsEnabled`, owner-verified). No D1 loose ends remain. **R5 (GDB read timeout) done & owner-verified 2026-07-08.** Still parked: **E1** (Options Apply dirty-tracking, running as background task `task_ea2834b6`).
...
- [ ] **Breakpoint repaint glitch** — *owner-reported 2026-07-08.* After setting a breakpoint, the rest of the code editor goes blank/invisible until the user clicks back into the form/editor window. Cosmetic repaint issue in the breakpoint-set path (likely a missing `EditControl` `Refresh`/`Invalidate` after the gutter change). Not investigated.
