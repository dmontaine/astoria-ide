...
**Last updated:** 2026-07-11 — **Fable review remediation nearly complete.** Wave 1 (T8–T13, hygiene) done; T1 decision gate resolved (owner chose **Option B — per-user install**, portable model kept, no APPDATA migration); Wave 3 (T3 PipeCmd rework, T4 native project delete, T7 unchecked writes, T2a/T2b portable-model safety net, T17 English-only sweep) all done and committed individually; T16 (Fable adversarial review) done with all 3 findings fixed; T5 (quote the bundled GDB launch path) done and owner-verified against a spaced path. **Remaining:** the debugger work (formerly T6) — now extracted into its own [Debugger Reliability sub-project](#-active-sub-project--debugger-reliability-queued-2026-07-11) (DR-1..DR-5) — plus the T15 theme-catalog owner memo and the accumulated owner smoke-test list (see the T16 row). An owner-found **"Add" dialog bug** (shared `frmPath`'s `Form_Close` clobbered `ModalResult` on every close, silently breaking every Add flow) was root-caused and fixed the same day. Separately, the debugger **Step Out** menu item was promoted to the top-level Run menu with clarifying step tooltips (2026-07-11). Full dated session-by-session narrative is archived in [HISTORY.md](HISTORY.md).
...
| 15 | 5 — close-out | **T14 — PROJECT_STATUS.md consolidation pass** — ✅ **DONE 2026-07-11 (Opus)** | Opus | F-C3 | Resolved the frmNewProject-icons vs. B2-closed contradiction (§6 + Open Items); merged the dated "Last updated" header pile into one current summary; refreshed the stale "Basic CI" item (now marked done, T8); archived the AI-removal task list and the C4 transform detail to [HISTORY.md](HISTORY.md); fixed the self-reported line count. **One piece deferred:** archiving *this* Fable-remediation section is held until the Fable queue closes (T15 memo + owner smoke-test list) — the section is still active. Note the debugger work (formerly T6, Wave 4) is now the separate [Debugger Reliability sub-project](#-active-sub-project--debugger-reliability-queued-2026-07-11) and closes on its own timeline |
