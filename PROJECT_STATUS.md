...
>
> **⏭️ CROSS-MACHINE HANDOFF (2026-07-12, AMD → Intel).** The owner is switching machines specifically to reproduce **DR-4** on Intel graphics — **`git pull` first thing** to get everything below. **Ready for you:** the `EC.Paint` trace (`7604d8d`) is already in `EditControl.bas` (gated by `bDbgTrace`, no rebuild-time cost). **Repro:** rebuild, open `Projects/Project3/Module1.bas` (plain single-pane `.bas`, no debug session needed), click a line, press **F9** — ~half the viewport should blank until refocus (click in the text area). Read `Settings\debug_trace.log` (git-ignored, local-only): find the `EC.Breakpoint` line, then the **`EC.Paint` right after it** (the toggle paint) vs the **next `EC.Paint`** (the refocus paint that fixes it). If `dwClientY` is smaller on the toggle paint than the refocus paint, `vlc1` is short and that's the confirmed cause — the fix is then a client-size refresh forced before `EditControl.Breakpoint` paints. If `dwClientY` is IDENTICAL on both (matching what the AMD baseline showed), the hypothesis is disproven and this needs fresh diagnosis on Intel specifically — don't force the AMD fix onto different data. **Also available to pick up: DR-16** (see above) — the `ChangeEnabledDebug`/`DeleteDebugCursor`-in-`deinit` half is mechanically simple (same stage-then-`FlushDebugOutputOnUI`-applies pattern as DR-7, just re-audit `deinit`'s full call graph first since it kept turning up more each time one more function was checked); the `load_file` modal-`MsgBox` half needs an error-flow design decision before touching (does closing the IDE mid-dialog still need to block?) — don't fix blind. **⚑ Gotchas:** (1) **rebuild + commit a RELEASE exe (`Compile.bat`) after every debugger-behavior source fix, immediately, before ending the session** — this AMD session lost time to exactly this: debug-build verification runs kept restoring the old tracked `astoria.exe` afterward (to avoid committing a bloated debug binary), so the committed exe went stale relative to source for a while and the owner briefly saw pre-DR-15 behavior return on a binary that just hadn't been rebuilt. Don't repeat it — release-rebuild is now committed after DR-15 (`07ea740`) and after DR-7 (`ce10b5c`), so the tracked exe is current as of this handoff. (2) `UseDebugger=false` in `astoria.ini` is STALE (written only on clean exit) — the live Run ▸ Use Debugger toggle is what counts. (3) A program with **no bound breakpoint runs to exit** (DR-8 removed break-at-entry); breakpoints on **comment lines** don't bind. (4) Trace logs are git-ignored/local-only — each machine accumulates its own. (5) Source files have MIXED line endings (LF-only lines inside CRLF files) — use byte-precise single-line `Edit` anchors, never large multiline string matches.
...
| **DR-4** | 🔜 **THE NEXT ITEM — clean repro as of 2026-07-12.** (Was: "split-view gutter blank, regressed, deferred.") | Toggle a breakpoint (F9 or gutter) and **~half the viewport text disappears** until any refocus (click in the text area) restores it. | **Old hypothesis DISPROVED (2026-07-12):** it is **NOT** `.frm` CodeAndForm-split-only and **NOT** theme-specific — it reproduces on a **plain single-pane `.bas`** (`Projects/Project3/Module1.bas`). So `PaintControl True` in `EditControl.Breakpoint` ([`EditControl.bas`](src/EditControl.bas:241)) simply is **not repainting the whole viewport** even in the simple single-pane case. Cosmetic/non-fatal (refocus restores). Not caused by the 2C `FECLine` capture change (that's a pure read added before the same `PaintControl True`). | **✅ INTEL TRACE CAPTURED (2026-07-12, `EC.Paint`) — geometry hypothesis DISPROVEN.** On Intel, the toggle's full paint reaches `EC.Paint` with **identical, healthy geometry to every restoring paint**: `dwClientY=526, dwCharY=16, vlc1=32, LinesCount=68, VScrollPos=0, path=GDI` — the paint loop covers all 32 visible lines. So `vlc1` is NOT short; the AMD "don't force the client-size fix" branch applies. **Refined root cause:** it's the **`bFull=True` full-repaint path specifically** — the gutter toggle (`PaintControl True`) blanks; the restoring **click** does a **partial** `PaintControl` (`bFull=False`) which renders fine (it BitBlts the still-good buffer). Both go through the same `GetDC`+full-unclipped-`BitBlt` (`EditControl.bas:5578`) and the off-screen `bufBMP` is the correct size, so the app-side code is correct → this is an **Intel GDI-driver quirk in the full-repaint path**. **⚠️ Constraint:** can't just revert to partial paint — `PaintControl True` is the `b7af611` fix for the split-view blank, so that would regress DR-4's original case. **Next diagnostic (instrument-first, not a guess-fix):** add `bFull`/`OnlyCursor` to the `EC.Paint` trace and log the **actual line count drawn** inside the `bFull=True` loop (does the loop draw all 32 to `bufBMP`, or does something clear/skip the lower half only under `bFull=True` on this GDI path?). Only then choose the fix. |
...
| **DR-16** | Residual worker-thread UI touches found while tracing `deinit`'s call graph during the DR-7 fix (**new 2026-07-12, not fixed**) | Two more instances of the DR-7/DR-3 hazard class (worker touches a UI control directly; `ThreadsEnter`/`ThreadsLeave` are no-ops) — neither has an observed symptom (like DR-7's residual, lower-risk) but both are genuine unmarshaled races. | (a) `load_file` ([`Debug.bas`](src/Debug.bas)) calls a **blocking modal `MsgBox`** directly from the worker thread ("could not find program to debug"). (b) `deinit()` — called from the worker on every session-end path — calls `ChangeEnabledDebug` (14+ toolbar/menu `.Enabled` writes) and `DeleteDebugCursor` (calls `.Repaint` on a control) directly. Each function checked in `deinit`'s call graph turned up another instance — stopped tracing further rather than chase the graph open-endedly without a checkpoint; there may be more. | **Not started.** (a) is NOT a simple stage-it fix like DR-7's — `load_file` currently blocks on the MsgBox and the caller depends on that (`Return -1` right after), so marshaling it needs an error-flow redesign, not just deferred text; decide the desired UX first (does closing the IDE mid-dialog still need to block?). (b) is mechanically the same pattern as DR-7 (stage a flag/params, apply in `FlushDebugOutputOnUI`) and should be easy to fold in — but re-audit `deinit`'s full call graph first rather than fixing piecemeal, since it kept turning up more each time it was checked. |
