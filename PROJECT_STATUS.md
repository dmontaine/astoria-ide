...
**Contingency if that check fails:** Development would have to fall back to `-gen gcc` + debug info too, at which point both modes use the same backend and the only remaining difference is debug-info-on/off + optimization level. In that scenario, `gas64` has no remaining reason to be in the picture at all and could likely be dropped from the project entirely — a simpler end state, just not the hoped-for one. Worth knowing going in that this is a real possible outcome, not just a formality to confirm.
