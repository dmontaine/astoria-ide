...
- [x] ~~**T0 (posture decision)**~~ — **RESOLVED by owner 2026-07-12: patch locally, always.** MFF is now a **separately maintained fork with no upstream sync** — all source (incl. modifications) is published on GitHub, satisfying LGPL's make-available obligation; the `COPYING.LGPL*` notices in the tree cover notice-preservation, and git history is the record of what the fork changed. So: (a) fix MFF freely in place, no upstream-compat constraint; the `UPSTREAM.md`/baseline-pin idea is **dropped as low-value** (no sync relationship to diff against). **One sub-decision still open, now purely internal (not upstream-driven):** whether to **drop the ~150 unused controls** (incl. `HTTPServer`/`HTTP`) from what compiles into `mff64.dll` — a binary-size / attack-surface / maintenance call. If dropped, the entire F-N* (HTTPServer/HTTP) task class disappears; if kept, T-OPUS-3 / T-SON-1 / T-SON-2 / T-HAIKU-1 apply. **Recommend deciding this before starting the F-N* tasks** so effort isn't spent hardening controls that might be removed.
