...
**Replace all of it with a single "Development" / "Final" choice** (exact label TBD, matches the §1 guiding-principle example almost exactly):
- **Development:** `-gen gas64`, debug info on, no optimization flags applied (gas64 doesn't take GCC-style `-Wc -O#` flags at all) — fast compile, immediately debuggable. This is the default for day-to-day edit/compile/run.
...
**`ToLLVM`/`ToCLANG` are fully removed under this design**, not merely deprioritized (§13.2, §4 above) — there's no reason to keep a third/fourth backend option around, hidden or otherwise, given the "no exposed compiler internals" stance.

**Infrastructure-risk concern raised and resolved (2026-07-03): bundling both backends is not "two compilers."** Owner asked whether supporting both `gcc` and `gas64` means maintaining two competing C compilers, and whether that plumbing risk should push toward picking just one. Clarified: **`gas64` is not a C compiler at all** — it's FreeBASIC's own internal code generator; `fbc64.exe` translates BASIC directly to x86-64 GNU assembler itself, with no C step. The only external tools it calls are `as.exe`/`ld.exe` (the GNU assembler/linker), which the `-gen gcc` path also needs regardless (GCC's own `cc1.exe` still has to assemble/link through something). So there's exactly one full C-compiler toolchain in play (GCC, for the `-gen gcc` path) — not two. And unlike the Clang/LLVM case above, **this plumbing isn't hypothetical — both paths are already proven and working in the current bundled `Compiler/` tree**: the pre-existing `MDIForm.vfp`/`Sudoku.vfp` project templates (predating this session) already default to `-gen gas64`, and `Compile.bat` itself already builds the IDE via `-gen gcc`. Nothing new needs bundling for either path. This means the Development/Final design's remaining open question is purely the GDB/gas64 debugging check above — not a "should we even attempt two backends" infrastructure question.

**Related cleanup found and done while investigating this (2026-07-03): leftover 32-bit GCC internals removed.** `Compiler/bin/libexec/gcc/i686-w64-mingw32/9.3.0/` was a complete parallel 32-bit GCC toolchain (its own `cc1.exe`, `as.exe`, `ld.exe`, plus 9 dependency DLLs — 31 MB, 12 files) sitting next to the real 64-bit one (`bin/libexec/gcc/x86_64-w64-mingw32/9.3.0/`). The earlier 32-bit-removal commit (`15e66cc`) only caught the top-level `Compiler/bin/win32/` folder and missed this deeper one. Confirmed zero references anywhere in build scripts or source before deleting; verified both `-gen gcc` (IDE self-build via `Compile.bat`) and `-gen gas64` (direct `fbc64` compile of `Examples/MDIForm`) still compile clean afterward. This also incidentally clarified the project's own internal structure: GCC's complete, self-contained internal toolchain (the actual C compiler, `cc1.exe`) lives under `bin/libexec/gcc/x86_64-w64-mingw32/9.3.0/`, separate from the top-level `bin/win64/` tools (`as`/`ld`/`ar`/`dlltool`/`GoRC`) that `fbc64` uses directly for the `gas64` path and for linking/resource-compiling regardless of backend — i.e., the gcc-specific and gas64-specific tooling are already naturally separated on disk, just not by folder-per-mode.

**Hard precondition, not a nice-to-have:** this design only works end-to-end if GDB can actually debug `-gen gas64`-compiled output — Development mode's entire premise is the edit/compile/debug loop. The still-open empirical check above (does GDB cleanly step/breakpoint/watch against gas64 output) needs a real answer before this ships, not an assumption.

**Decided (owner, 2026-07-03): try both, and let the empirical check settle it.** Implement Development (`gas64`) and Final (`gcc`) as designed above; if GDB turns out not to debug `gas64` output cleanly, **remove `gas64` from the project entirely** rather than keep a degraded/unreliable Development mode around. Owner's read on why this gap might not already be a well-known FreeBASIC community issue despite `gas64` having real users: plenty of BASIC hobbyist code gets debugged by print-statements rather than an actual debugger, so a GDB/gas64 incompatibility could easily go unnoticed by most of that user base. If the check fails, both modes converge to `-gen gcc` (debug-info-on/off + optimization level being the only remaining difference) — a simpler end state, just not the hoped-for one.
