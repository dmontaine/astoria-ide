...
**D1 form-close greying gap — now fixed** (see the D1 outcome section above; resolved via `ChangeMenuItemsEnabled`, owner-verified). No D1 loose ends remain. Still parked from the Opus backlog: **R5** (GDB read timeout, needs a live-repro session) and **E1** (Options Apply dirty-tracking, running as background task `task_ea2834b6`).
