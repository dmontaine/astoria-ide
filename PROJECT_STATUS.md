...
**Dark mode — KNOWN BROKEN when actually enabled (2026-07-04, unresolved, picking up next session):** the checkbox and persistence are correct, and the app runs fine with the setting off (the shipped default). But turning it on genuinely crashes the app — confirmed via repeated reproduction, not a one-off. Two real bugs were found and fixed this session while investigating, and a third remains open:
...
**Current safe state:** `DarkMode` defaults to (and is currently persisted as) `false`; the checkbox is left visible in the UI per owner's explicit call ("no one else is using this and we know not to enable it") rather than re-hidden — anyone who does check it and hits Apply should expect a crash until this is resolved. Do not remove the workaround note or the checkbox without re-verifying the crash is actually fixed.
...
### Next session start here — Form Designer panel/layer navigation (part b) (raised 2026-07-03, part a shipped 2026-07-04)
...
- **(b) Panel/layer navigation — not started, this is the real next step.** Owner clarified 2026-07-04 (after testing (a) on `MDINotepad`'s `frmCodePage.frm`, which surfaced and got several iterations of feedback — see git log): the concrete case that motivated this whole investigation was a **layered canvas-style control** in `frmOptions.frm` for the Editor itself, not `PagePanel` specifically — "any component that allows layering needs to have a visual way to navigate through the layers." The small spinner control *is* visible in that case, "it just didn't work." Confirmed earlier in this same investigation: `PagePanel`'s spinner (`NumericUpDownControl`/`UpDownControl_Changing`, `PagePanel.bas:277-291`) really does drive real panel-visibility switching when written through in design mode (`WriteProperty("SelectedPanelIndex"/"SelectedPanel", ...)`, `PagePanel.bas:38-43,203-233`) — but nothing that *selects* a control (old `cboClass_Change`, or the new control tree's `SelectControlTreeNode`) ever calls it; `Designer.BringToFront` (`Designer.bas:2082`) only fixes Z-order, never touches `SelectedPanelIndex`/`SelectedPanel`. **Next session:** find the actual layered control in `frmOptions.frm` that prompted this (owner said "canvas", not `PagePanel` — could be a different/custom layering mechanism, needs identifying first), then wire selection to flip the ancestor layer visible before `MoveDots`, and separately decide whether the switch mechanism itself needs to become more discoverable (visible tabs) or just needs the existing spinner/menu fixed to actually work.

**Recurring pattern worth remembering:** "sound design, unfinished last-mile integration" has now shown up three times in two days (docking engine, `PagePanel` panel-switching, and likely whatever layering mechanism `frmOptions.frm`'s canvas control uses). Owner's framing: "There is a lot to like about it, just that last 10% that takes 70% of the time and effort never got done." Treat this as a signature of this codebase generally, not a one-off — foundational pieces tend to be genuinely well thought out; look for missing integration/polish before assuming something needs to be rebuilt from scratch.
