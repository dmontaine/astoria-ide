...
- [ ] **H-3 (M, only IDE-relevant one) — `Control` `WM_PAINT` uses `GetDC` not `BeginPaint`/`EndPaint` (`Control.bas:897-908`).** The `OnPaint` path doesn't clip to or validate the invalid region — it repaints the whole client area on every partial invalidation, and avoids an infinite `WM_PAINT` storm *only* by falling through to `DefWindowProc` (so anyone who later makes the case `Return`/`Handled=True` triggers a CPU-pegging repaint loop). Fix: `BeginPaint`/`EndPaint` (or `ValidateRect`) on the `OnPaint` path; live-verify custom-control paint. **(Opus)**
