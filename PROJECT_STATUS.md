...
**New open item (owner-reported 2026-07-08):** **Options ▸ Code Editor ▸ Defaults page is empty** — pre-existing, unrelated to C4. `pnlDefaults`/`grbDefaults`/`vbxDefaults` exist and are wired into the tree (`tnEditor->Nodes.Add(("Defaults"), "Defaults")`, `.pnlDefaults.Visible = Key = "Defaults"`), but the page apparently has no populated content. Owner said it "can be removed" — not investigated further; a future session should either populate it or remove the tree node + panel per that guidance.
