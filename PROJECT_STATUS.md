...
> ⏸️ **MID-DEBUG-SESSION HANDOFF (2026-07-11, WIP — synced across machines via git).** We are in the middle of **T6 (debugger pipe-race, attempt #2)**. The `set_bp` fix is committed and compiles clean, but the **owner live-verify FAILED — two debugger bugs surfaced during testing** (both pre-existing, not caused by the `set_bp` change, which only touches the F9 path):
> 1. **Step Into doesn't highlight / doesn't scroll to the current line** — on a *single deliberate step*: the viewport does not move to the debug line, and the line is not highlighted even after manually scrolling to it; subsequent steps also don't highlight. This is a **separate bug** from #2 (the `fcurlig` → `TimerProcGDB` → `CurExecutedLine`/`SetSelection` highlight handoff is not producing a highlight — see [Main.bas:3028](src/Main.bas) `TimerProcGDB` and [Debug.bas:765](src/Debug.bas) `line_highlight`).
> 2. **Rapid F8 (Step Into) hangs the app** — root-caused statically to the worker loop dequeuing/sending a new command while `Running=True` ([Debug.bas:2056](src/Debug.bas)): steps get sent back-to-back, then `RefreshDebugPanelsAfterStop` reads the leftover step annotations instead of its own responses → read-stream desync → a `readpipe` blocks forever.
>
> **Next step (per T6 charter): instrument-first.** Re-add focused file-trace (worker dequeue/read/write, `line_highlight` outcome, `TimerProcGDB` action), reproduce both on the owner's machine, confirm, then fix precisely — the hang fix is a worker-loop **lockstep reorder** (finish one command's stop-read + panel-refresh before sending the next); the highlight fix needs the trace to pin why the `fcurlig`/timer handoff produces no paint. **Do NOT reorder the worker loop without a confirming trace** (attempt #1 hard-locked doing an unverified worker-loop change). See the "Breakpoint-during-debug pipe race" Open Item for the full write-up.

**Last updated:** 2026-07-11 — **Fable review remediation nearly complete.** Wave 1 (T8–T13, hygiene) done; T1 decision gate resolved (owner chose **Option B — per-user install**, portable model kept, no APPDATA migration); Wave 3 (T3 PipeCmd rework, T4 native project delete, T7 unchecked writes, T2a/T2b portable-model safety net, T17 English-only sweep) all done and committed individually; T16 (Fable adversarial review) done with all 3 findings fixed; T5 (quote the bundled GDB launch path) done and owner-verified against a spaced path. **Remaining:** T6 (`set_bp` pipe-race, attempt #2 — Wave 4), the T15 theme-catalog owner memo, and the accumulated owner smoke-test list (see the T16 row). An owner-found **"Add" dialog bug** (shared `frmPath`'s `Form_Close` clobbered `ModalResult` on every close, silently breaking every Add flow) was root-caused and fixed the same day. Separately, the debugger **Step Out** menu item was promoted to the top-level Run menu with clarifying step tooltips (2026-07-11). Full dated session-by-session narrative is archived in [HISTORY.md](HISTORY.md).
