...
- [ ] **T-DEFER (MFF) — not blocking:** standalone-widget trimming (F-H3 — **~15–20 droppable widgets, NOT ~150**; see the T0 note above for the keep/drop list — most non-included modules are foundational infra that can't be dropped), CN/example baggage (F-H2), `Canvas` GDI-handle audit if long-session handle growth is ever observed (F-R-gdi). Revisit only with owner appetite or an observed symptom.
