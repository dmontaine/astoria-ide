...
### Code-stripping pass: remove dead alternative-debugger/compiler-backend support (2026-07-03, in progress)

With `gas64` confirmed dead and Clang/LLVM never bundled, the following are now dead code, not just unused options — being physically removed, not hidden, per this project's established dead-code discipline (§3a):

- **`Debug.bas` — the "Integrated" (stabs) debugger.** Exists specifically to pair with `-gen gas/gas64 -g` output, which has just been shown not to contain the stabs data it expects. This is the single largest, riskiest removal here — `Debug.bas` is the biggest file in the project (~12,000 lines) and mixes this debugger's code with the GDB debugger's code throughout.
- **`TabWindow.bas`/`BuildService.bas` — `ToGAS`/`ToLLVM`/`ToCLANG` compile-backend branches**, plus the `optOptimizationFastCode`/`optOptimizationLevel`/`optOptimizationSmallCode` command-line construction, replaced by the fixed Development/Final optimization defaults above.
- **`VisualFBEditor.bas` — the `IntegratedGDBDebugger` vs. non-GDB branches** in the debug-action dispatch (`Case "Start"`, `"StepInto"`, `"End"`, etc. — 11 occurrences per the earlier complexity-audit count) collapse to GDB-only, since there's no second debugger path left to branch on.
- **`frmProjectProperties.frm` — the six Project Properties controls** (`optCompileToGas`/`optCompileToGcc`/`optCompileToLLVM`, `optOptimizationFastCode`/`optOptimizationLevel`/`optOptimizationSmallCode`, `chkCreateDebugInfo`) collapse into the single Development/Final choice designed above.

**Full change-surface mapping and execution log:** see the session notes below this entry as the pass progresses — this is being done in stages with a compile check after each stage, not as one blind pass, given `Debug.bas`'s risk profile.

**Companion note (owner, 2026-07-03):** once this code-level pass lands, do a dedicated **UI/settings sweep** for GTK, Linux, and alternative-compiler/debugger remnants specifically in the visible UI (not just source branches) — see the concrete note under §13.2, since deleting a code branch doesn't always mean its Project Properties/Options control gets deleted too.
