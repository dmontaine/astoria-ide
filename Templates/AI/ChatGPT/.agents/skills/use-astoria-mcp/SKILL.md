...
The Codex CLI reads MCP servers from its config file (`~/.codex/config.toml`). Add an
entry pointing at `astoria-mcp.exe` (it sits beside `astoria.exe` in your Astoria install):
...
Verify the exact key names against the current Codex MCP docs, then restart Codex so it
connects. See `AGENT_MCP_SETUP.md` in the Astoria install for the full reference.
