...
- **Save source as UTF-8 with NO byte-order mark.** FreeBASIC treats a leading BOM as an instruction to make string literals wide, so a BOM'd file can compile and then print garbled console output. Astoria strips the BOM on save; direct edits must also be BOM-less.
...
- `Str(a = b)` prints `0`/`-1`, not `"false"`/`"true"`; a comparison yields an integer. `Str()` of an actual `Boolean` does return `"true"`/`"false"`.
- **Never `ReDim Preserve` an array whose element type owns heap memory** (a UDT containing `String`/`WString`, or the framework's `UString`). It shallow-copies owned buffers and can cause a delayed double free. Use a list type or a non-owning representation instead.
...
- **Keep `project.astoria`.** It is the canonical project description and Astoria project marker, and must retain `AstoriaProject=1`. It records project, Git, and AI choices and supplies the remote used by Astoria's Git features. Edit it through **Project > Edit Project Description** or as UTF-8 `Key=Value` lines. See **git-workflow**.
...
- **Version control:** a Git-backed project has `.gitignore`/`.gitattributes`, with its SSH remote recorded in `project.astoria`. Commit only when asked, with clear scoped messages; never push or force-push unless explicitly asked. See **git-workflow**.

### Editing discipline

- **If Astoria is running with this project open, edit through the `astoria` MCP server** (`write_file`, `set_active_file_content`) instead of changing open files behind the IDE. This keeps the IDE buffer authoritative and avoids reload conflicts.
...
- **Verify by effect, not only by return value.** Prove the observable result: re-read the row, inspect output, count records, or confirm the file exists.
- **Make the assertion as strong as the claim.** A window opening does not prove the application works; assert something only the working feature produces.
- **Measure before theorising.** Capture the relevant value, state, or trace before building a diagnosis from code reading.
- After a fix, re-run the scenario that exposed it. A successful compilation alone is not a regression test.

## Skills (task playbooks)

Detailed playbooks live in `.agents/skills/` and load on demand. They cover MCP-driven IDE work, Git workflow, building/running, compile errors, modules, forms, controls/events, resources, safe form editing, framework discovery, manifest audits, refactoring, debugging, WinAPI interoperability, and release preparation.
...
1. Read `GitURL=`, `GitProvider=`, `GitUserName=`, and `GitEmail=` from `project.astoria`; do not guess. An empty `GitURL=` means no remote is configured.
2. Use **Project > Git Commit/Push** or the Git CLI. Astoria uses SSH remotes.
3. If SSH authentication fails, direct the user to `Templates\Git\sshkeys.md`; do not silently replace the remote with HTTPS.
4. Commit only when asked. Never force-push or rewrite published history.

---
*Generated by Astoria for ChatGPT (Codex). Expand with what this project does and how you like to work.*
