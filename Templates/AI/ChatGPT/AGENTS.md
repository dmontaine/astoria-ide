...
- **Keep `project.astoria`.** It is the canonical project description and Astoria project marker, and must retain `AstoriaProject=1`. It records project and AI choices. Edit it through **Project > Edit Project Description** or as UTF-8 `Key=Value` lines.
...
Detailed playbooks live in `.agents/skills/` and load on demand. They cover MCP-driven IDE work, building/running, compile errors, modules, forms, controls/events, resources, safe form editing, framework discovery, manifest audits, refactoring, debugging, WinAPI interoperability, and release preparation.
