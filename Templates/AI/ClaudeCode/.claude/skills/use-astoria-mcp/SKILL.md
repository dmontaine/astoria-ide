...
Claude Code auto-discovers a project-scoped **`.mcp.json`** at the repo root. This template
ships one. If its `command` still contains the `{{ASTORIA_MCP_EXE}}` placeholder, replace
it with the full path to `astoria-mcp.exe` (it sits beside `astoria.exe` in your Astoria
install), or run:
...
Then restart Claude Code so it picks up the server. See `AGENT_MCP_SETUP.md` in the Astoria
install for the full reference.
