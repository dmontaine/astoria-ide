...
- **`project.astoria`** (project root) is the canonical description file and the marker that identifies this folder as an Astoria project -- it must keep its `AstoriaProject=1` line. It records the creation choices (author, license, description, Git provider/username/email/remote URL, AI settings) and is what the IDE's **Edit Project Description** and **Git Commit/Push** features read from (the `.vfp` mirrors some of the same keys). Don't delete or rename it; edit it via **Project menu > Edit Project Description**, or by hand as line-based `Key=Value` (UTF-8). See the **git-workflow** skill.
...
- **Version control:** a git-backed project (`Mode=ExistingGit`/`UseGit=true` in `project.astoria`) ships a `.gitignore`/`.gitattributes` pair, and its remote is recorded in `project.astoria` (`GitURL=`, an SSH URL). Commit/push via **Project menu > Git Commit/Push** or the git CLI over SSH -- see the **git-workflow** skill. Commit only when asked, with clear scoped messages; never push or force-push unless explicitly asked.
...
The shared playbooks below are also spelled out here as a fallback.

| Skill | When to use |
|---|---|
| `use-astoria-mcp` | Drive the live IDE via MCP (preferred when connected) |
| `git-workflow` | Commit/push a git-backed project (`project.astoria`) |
...
mirrors are in `.cursor/rules/freebasic-tasks.mdc`. MCP config: `.cursor/mcp.json`.

### Build and run the project

1. If the `astoria` MCP server is connected: use `build` / `run` / `get_errors` (see **use-astoria-mcp**).
2. Otherwise, in the Astoria IDE: open `{{PROJECT}}.vfp`, press **F5**.
3. Console CLI alternative: `fbc -s console <MainFile>.bas`, then run the `.exe` produced next to it.
4. GUI CLI alternative: add `-i <AstoriaDir>\Controls\Framework`; see the Build-and-run rules above for the `.frm`/`.rc` caveats -- the IDE is the reliable path.
5. On failure, read the first `file(line) error N:` message and fix that line first.
...
### Git: commit and push (git-backed projects)

For a project with a Git remote (`GitURL=` set in `project.astoria`):

1. Read the remote and identity from `project.astoria` (`GitURL=`, `GitProvider=`, `GitUserName=`, `GitEmail=`) -- don't guess one. An empty `GitURL=` means there's no remote yet.
2. Commit/push either through the IDE's **Project menu > Git Commit/Push**, or on the command line:
   ```
   git add -A
   git commit -m "clear, scoped message"
   git push
   ```
3. Astoria uses **SSH** remotes (`git@host:user/repo.git`); a push that fails on a permission or host-key error means no SSH key is set up for the provider -- point the user to `Templates\Git\sshkeys.md`, don't switch the remote to HTTPS.
4. Commit only when asked; never force-push or rewrite published history. Don't commit `Temp/`, `.exe` files, or `{{PROJECT}}_Change.log` (already covered by `.gitignore`). Keep `project.astoria` -- it's the Astoria marker and the source of the remote.

---
*Generated by Astoria for Cursor. Expand with what this project does and how you like to work.*
