...
Point Kun at the Astoria MCP server per its MCP configuration. The server is a standard
MCP stdio server: run `astoria-mcp.exe` (it sits beside `astoria.exe` in your Astoria
install) as the server command with no arguments. Confirm the exact configuration format
in Kun's current MCP docs. See `AGENT_MCP_SETUP.md` in the Astoria install for the full
reference.
