...
OpenCode reads MCP servers from the **`mcp`** block in `opencode.json`. This template ships
one (`mcp.astoria`, a `local` server). If its `command` still contains the
`{{ASTORIA_MCP_EXE}}` placeholder, replace it with the full path to `astoria-mcp.exe` (it
sits beside `astoria.exe` in your Astoria install), then restart OpenCode so it connects.
Verify the exact `mcp` shape against OpenCode's current docs. See `AGENT_MCP_SETUP.md` in
the Astoria install for the full reference.
