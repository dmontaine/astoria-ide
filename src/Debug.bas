...
Private Function var_sh2(t As Integer, pany As UInteger, p As UByte, sOffset As String, fxlen As Integer) As String
	...
	Else
		...
		While 1
			...
			'If recupstab.stabs Then
				...
				'		If procnmt="main" Then flagstabd=True ' + TODO suppress equivalent below
				...
				'		' ==
				'		'If InStr(recup,":") Then 'new include file path name with file name
				'		'	sourcenb+=1:source(sourcenb)=recup:sourceix=sourcenb' Utility: sourcead(sourcenb)=recupstab.ad
...
Sub RunWithDebug(Debugger As String, ByRef ProjectFileName As WString, ByRef ProjectCommandLineArguments As WString, ByRef MainFile As WString, ByRef CompileLine As WString, ByRef FirstLine As WString)
	...
	If WGet(DebuggerPath) <> "" Then
		...
		If Idx <> -1 Then
			...
			Dim As UString cmdArg
			If InStr(LCase(WGet(DebuggerPath)), "gdb") Then cmdArg = "" Else cmdArg = GetFileName(exename)
			WLet(CmdL, Tool->GetCommand(cmdArg))
		End If
	End If
		WatchIndex = -1
	'#ifdef __USE_GTK__
