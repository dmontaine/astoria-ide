...
' Pounds to RTF(Pounds to half-points)
		...
		Else
			...
			Case "`"  ' 1. Code block processing (enhanced robustness)
				If LCase(Left(*Lines(i), 12)) = "```freebasic" OrElse LCase(Left(*Lines(i), 5)) = "```fb" OrElse LCase(Left(*Lines(i), 5)) = "```vb" Then
					...
					Continue For
				End If
			Case "1" To "9" ' 2. Parse numbered title lists like "1. **title 1**"
				...
				If dotPos > 0 Then
					...
					' Format title: bold, larger font (cf15 = Keywords color)
					WAdd(rtfiText, "\b\fs32\cf15 " & listNum & ". " )
					inListItem = True ' Mark list item; following indented lines belong to this list
				...
				Else
					' If not the specified Title format, treat as normal text
				...
				'	Continue For
				'End If
			Case " " ' 3. Handle indented content below list items (e.g. "   abcd")
				If inListItem Then
					Dim As Integer indentCount = 0
					While indentCount < Len(*Lines(i)) AndAlso Mid(*Lines(i), indentCount + 1, 1) = " "
						indentCount += 1
					Wend
					' Convert to RTF indent (~360 twips per 3 spaces, plus 360 list offset)
...
Function EscapeRTF(ByRef iText As WString) As WString Ptr
	...
	For i As Integer = 0 To iLen - 1
		...
		End If
		Select Case iText[i]
		Case 92, 123, 125, 126, 94  ' ASCII code: \ { }   ^
			(*ResultPtr)[Posi] = 92
			(*ResultPtr)[Posi + 1] = iText[i]
			Posi += 2
		Case 13  ' ASCII code: Cr LF
			If i < iLen - 1 AndAlso iText[i + 1] = 10 Then
				i += 1  ' Skip following line feed (LF)
			End If
			(*ResultPtr)[Posi] = 10
			Posi += 1
			'If i < iLen - 1 AndAlso iText[i + 1] = 10 Then
			...
			'         Posi += 4
		Case 10  ' ASCII code: Cr LF
			(*ResultPtr)[Posi] = 10
			Posi += 1
		Case 9  ' ASCII code: TAB
			(*ResultPtr)[Posi] = 32
			Posi += 1
			(*ResultPtr)[Posi] = 32
			Posi += 1
