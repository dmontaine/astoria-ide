...
Private Sub frmAddProcedureType.cmdOK_Click(ByRef Sender As Control)
	...
	Case Else
		...
		Else
			Select Case True
			Case optConstructor.Checked: sType = "Sub": sConstructorDestructor = " Constructor" & IIf(txtPriority.Text = "0", WStr(""), " " & Trim(txtPriority.Text))
			Case optDestructor.Checked: sType = "Sub": sConstructorDestructor = " Destructor" & IIf(txtPriority.Text = "0", WStr(""), " " & Trim(txtPriority.Text))
			End Select
		End If
	End Select
	...
	Else
		...
		If LineEndType > -1 Then
			...
			ptxtCode->InsertLine LineToAdd + q2, SpaceStr & !"\tDeclare" & IIf(optStaticMember.Checked, WStr(" Static"), IIf(optAbstractMember.Checked, WStr(" Abstract"), IIf(optVirtualMember.Checked, WStr(" Virtual"), WStr("")))) & " " & sType & IIf(optConstructor.Checked OrElse optDestructor.Checked, WStr(""), " " & txtName.Text) & IIf(txtAlias.Text = "", WStr(""), " Alias """ & txtAlias.Text & """") & IIf(StartsWith(txtParameters.Text, "(") OrElse Trim(txtParameters.Text) = "", txtParameters.Text, "(" & txtParameters.Text & ")")
			If ptxtCode = @tb->txtCode Then q1 += 1
		End If
		tb->txtCode.InsertLine i + q1, ""
	End If
	If Not optAbstractMember.Checked Then
		tb->txtCode.InsertLine i + q1 + 1, IIf(optPublicScope.Checked, WStr("Public "), IIf(optPrivateScope.Checked, WStr("Private "), WStr(""))) & IIf(chkProcedureAlso.Checked, IIf(optStaticMember.Checked, WStr("Static "), IIf(optVirtualMember.Checked, WStr("Virtual "), WStr(""))), WStr("")) & sType & " " & IIf(bInsideType AndAlso (optConstructor.Checked OrElse optDestructor.Checked), txtName.Text, sTypeNameDot & txtName.Text) & IIf(txtAlias.Text = "", WStr(""), " Alias """ & txtAlias.Text & """") & IIf(StartsWith(txtParameters.Text, "(") OrElse Trim(txtParameters.Text) = "", txtParameters.Text, "(" & txtParameters.Text & ")") & sConstructorDestructor & IIf(chkStatic.Checked, WStr(" Static"), WStr("")) & IIf(chkExport.Checked, WStr(" Export"), WStr(""))
