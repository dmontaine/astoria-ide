...
Private Sub frmAddTypeType.cmdOK_Click(ByRef Sender As Control)
	...
	If Not bInsideType Then
		...
		If txtDescription.Text <> "" Then
			...
			Next
		End If
		tb->txtCode.InsertLine i + q1 + 1, IIf(optPublicScope.Checked, WStr("Public "), IIf(optPrivateScope.Checked, WStr("Private "), WStr(""))) & sType & " " & txtName.Text & IIf(txtAlias.Text = "", WStr(""), " Alias """ & txtAlias.Text & """") & IIf(cboExtends.ItemIndex = 0, WStr(""), " Extends " & cboExtends.Text) & IIf(txtField.Position = 0, WStr(""), " Field = " & txtField.Text)
		If optClass.Checked AndAlso chkRedefineClassKeyword.Checked Then
	...
	Else
		...
		If LineEndType > -1 Then
			...
			If txtDescription.Text <> "" Then
				...
				Split(txtDescription.Text, Chr(13) & Chr(10), res())
				For j As Integer = 0 To UBound(res)
					ptxtCode->InsertLine LineToAdd + q2 + j, SpaceStr & IIf(bAddSpaces, WStr(!"\t"), WStr("")) & "'" & res(j)
				Next
				q2 += UBound(res) + 1
			...
			CheckBi(ptxtCode, txtCodeBi, ptxtCodeBi, tb)
			ptxtCode->InsertLine LineToAdd + q2, SpaceStr & IIf(bAddSpaces, WStr(!"\t"), WStr("")) & IIf(optPublicScope.Checked, WStr("Public "), IIf(optPrivateScope.Checked, WStr("Private "), WStr(""))) & sType & " " & txtName.Text & IIf(txtAlias.Text = "", WStr(""), " Alias """ & txtAlias.Text & """") & IIf(cboExtends.ItemIndex = 0, WStr(""), " Extends " & cboExtends.Text) & IIf(txtField.Position = 0, WStr(""), " Field = " & txtField.Text)
			ptxtCode->InsertLine LineToAdd + q2 + 1, SpaceStr & IIf(bAddSpaces, WStr(!"\t"), WStr("")) & !"\t"
			ptxtCode->InsertLine LineToAdd + q2 + 2, SpaceStr & IIf(bAddSpaces, WStr(!"\t"), WStr("")) & "End " & sType
			i = LineToAdd - 2
			If ptxtCode = @tb->txtCode Then q1 += 1
		End If
	End If
